...
public class DateHelper {
	 ...
	 * @return
	 */
	public static String getTime(Object mins) {
		if (mins == null)
			return null;
		Date now = new Date();
		now = new Date(now.getTime() + (Integer.parseInt(String.valueOf(mins)) * 60 * 1000));
		return new SimpleDateFormat(DATE_FORMAT_DEFAULT).format(now);
	}
	...
	public static Date parseDate(Object date) throws ParseException {
		...
		if (date instanceof Date) {
			...
			return new SimpleDateFormat(DATE_FORMAT_DEFAULT).parse(date.toString());
		}
		return null;
	}
	/**
	...
	public static Date processTime(Object mins) {
		if (mins == null)
			return null;
		if (StringHelper.isEmpty(String.valueOf(mins)))
			return null;
		Date now = new Date();
		return new Date(now.getTime() + (Integer.parseInt(String.valueOf(mins)) * 60 * 1000));
	}
