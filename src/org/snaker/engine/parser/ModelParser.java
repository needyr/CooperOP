...
public class ModelParser {
	 ...
	 * @param bytes
	 */
	public static ProcessModel parse(byte[] bytes, Map<String, Object> map) {
				...
				for(int i = 0; i < nodeSize; i++) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						if("subprocess".equals(node.getNodeName())){
							if(map!=null){
								if(map.get(e.getAttribute(NodeParser.ATTR_NAME)) !=null){
									e.setAttribute("processName", (String)map.get(e.getAttribute(NodeParser.ATTR_NAME)));
								}else if(map.get("processname") != null){
									e.setAttribute("processName", (String)map.get("processname"));
								}
							}
						}
						NodeModel model = parseModel(e);
