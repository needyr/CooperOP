...
public abstract class AbstractDBAccess implements DBAccess {
	...
	public void updateTask(Task task) {
		if(isORM()) {
			...
			if(CommonFun.isNe(task.getCreateTime())){
				...
				Object[] args = new Object[]{task.getCreateTime(),task.getFinishTime(), task.getOperator(), task.getVariable(), task.getExpireTime(), task.getActionUrl(), task.getId(), task.getVersion() };
				int[] type = new int[]{Types.VARCHAR,Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,  Types.VARCHAR, Types.VARCHAR,  Types.VARCHAR, Types.INTEGER};
				saveOrUpdate(buildMap("update wf_task set create_Time=?,finish_Time=?, operator=?, variable=?, expire_Time=?, action_Url=?, version = version + 1 where id=? and version = ?", args, type));
			}
		}
	}
