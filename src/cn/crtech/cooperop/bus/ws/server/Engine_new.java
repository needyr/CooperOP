...
public class Engine_new {
	...
	private void onMessage(String message_id) {
		if (messagequenes.containsKey(message_id)) {
			...
			try {
				Object rtn = connection.onMessage(message);
				try {
				...
				try {
					connection.getSession().getBasicRemote().sendText(CommonFun.object2Json(error));
				} catch (Exception ex) {
					...
					if (tokens != null) {
						for (String token : tokens) {
							...
							if (c != null && c.getAppId().equals(connection.getAppId())) {
								try {
									c.getSession().getBasicRemote().sendText(CommonFun.object2Json(tmsg));
								} catch (Exception e) {
	...
	public static void sendMessage(String token, Message message) throws Exception {
		...
		connection.getSession().getBasicRemote().sendText(CommonFun.object2Json(message));
	}
