...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="DP_CCYJ01" init-action="" description="住院普通药品超常预警报表" system_product_code="hospital_oc" title="住院普通药品超常预警报表" remark="住院普通药品超常预警报表" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("hospital_oc.chart.chart.DP_CCYJ01", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("hospital_oc.chart.chart.DP_CCYJ01", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "hospital_oc.chart.chart.DP_CCYJ01", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("hospital_oc.chart.chart.DP_CCYJ01", null);
	}
...
<s:row type="frow"><s:textfield label="科室名称" cols="1" name="dept_name" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="zl_select_DP_CCYJ01_01" out_action="" tableid="" expressions="" maxlength="128" precision="">${empty dept_name ? '' : dept_name}</s:textfield>
</s:row>
...
<s:table.field name="drug_name" value="${empty drug_name ? '' : drug_name}" fdtype="字符" format="" dictionary="" label="药品名称" size="128" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="" is_href="" app_field=""></s:table.field>
...
<s:table.field name="操作" value="$[操作]"  fdtype="字符" format="" dictionary="" label="操作" size="20" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="textfield" align="left" width="120" fieldorder="" is_href="hospital_oc.chart.chart.DP_CCYJ01_02" app_field="" datatype="template"><s:textfield readonly="true" name="操作" value="$[操作]"  fdtype="字符" format="" dictionary="" size="20" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="textfield" align="left" width="120" fieldorder="" is_href="hospital_oc.chart.chart.DP_CCYJ01_02" app_field="">null</s:textfield>
</s:table.field>
</s:table.fields>
</s:table>
</s:row>
