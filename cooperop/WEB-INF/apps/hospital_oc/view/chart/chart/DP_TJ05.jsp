...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="DP_TJ05" init-action="" description="住院病人病种分布" system_product_code="hospital_oc" title="住院病人病种分布" remark="住院病人病种分布" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("hospital_oc.chart.chart.DP_TJ05", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("hospital_oc.chart.chart.DP_TJ05", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "hospital_oc.chart.chart.DP_TJ05", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("hospital_oc.chart.chart.DP_TJ05", null);
	}
...
<s:textfield label="科室名称" cols="1" name="dept_name" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="zl_select_DP_TJ05_01" out_action="" tableid="" expressions="" maxlength="128" precision="">${empty dept_name ? '' : dept_name}</s:textfield>
</s:row>
</s:form>
</s:row>
<s:row><s:table label="住院病种统计" cols="4" color="grey-silver" isdesign_="Y" tableid="CXTJ05_01" icon="" djselect="" subpage_name="" autoload="true" limit="100" sort="false" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="diagnosis_desc" value="${empty diagnosis_desc ? '' : diagnosis_desc}" fdtype="字符" format="" dictionary="" label="                   病种" size="60" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="320" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="cyrs" value="${empty cyrs ? '' : cyrs}" fdtype="实数" format="" dictionary="" label="           出院人数" size="60" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="right" width="120" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="pjzyts" value="${empty pjzyts ? '' : pjzyts}" fdtype="实数" format="" dictionary="" label="         平均住院天数" size="60" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="right" width="120" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="charges" value="${empty charges ? '' : charges}" fdtype="实数" format="" dictionary="" label="          金额" size="60" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="right" width="120" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="charges_ave" value="${empty charges_ave ? '' : charges_ave}" fdtype="实数" format="" dictionary="" label="               人均费用" size="20" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="right" width="120" fieldorder="" is_href="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:row>
</s:page>
