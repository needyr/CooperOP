...
function preCheckMsg(inputData, callback){
	...
	*/
	var show = SIMPLEINFO + '?drug_code=' + inputData.request.rows.row.order_code;
	var child_ret = window.showModalDialog(show, window, 'resizable:yes;scroll:yes;status:no;help:no;dialogWidth:934px;dialogHeight:665px;alwaysRaised:yes;location:no;');
	...
	$.ajax({
		...
		"success" : function(rtn) {
			...
			if(rtn.state == "Y"){
				...
				var show = ycbstransit + '?id=' + rtn.id + '&patient_id='
				  + inputData.request.patient.id + '&visit_id='
				  + inputData.request.patient.visitid 
				  + '&doctor_no=' + inputData.request.doctor.code
				  + '&stype=simic'
