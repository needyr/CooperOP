...
<s:page title="选择手术" disloggedin="false">
<s:row>
<div class="top">
	<input type="text" name="search" class="search" placeholder="数据量较大，请输入检索内容"/>
...
<div class="left">
	<div class="thead_Div" id="main_thead">
		<table class="mytable">
			<thead>
				<tr>
					<th width=100px>手术代码</th>
					<th width=150px>手术名称</th>
					<th width=100px>输入码</th>
				</tr>
			</thead>
		</table>
	</div>
...
if('${param.code}'){
	var code = '${param.code}';
	$.call("hospital_common.dicthisoper.searchCheck",{code: code.split(',')},function(rtn){
		if(rtn.oper){
			var html = [];
			var v = rtn.oper;
			...
			html.push('<td width=150px>'+v[i].operation_name+'</td>');
...
function search1(){
	...
	$.call("hospital_common.dicthisoper.search",{data: data},function(rtn){
		if(rtn.oper){
			var html = [];
			var v = rtn.oper;
			for(i=0;i<v.length;i++){
