...
<s:page title="选择给药途径" disloggedin="false">
<s:row>
<div class="top">
...
if('${param.code}'){
	var code = '${param.code}';
	$.call("hospital_common.dicthisroutename.searchCheck",{code: code.split(',')},function(rtn){
		if(rtn.routename){
			var html = [];
			var v = rtn.routename;
			for(i=0;i<v.length;i++){
				html.push('<tr class="is_c'+v[i].routename_code+'" data_id="'+v[i].routename_code+'">');
				html.push('<td width=100px>'+v[i].routename_code+'</td>');
				html.push('<td width=100px>'+v[i].routename_name+'</td>');
				html.push('</tr>');
			}
...
function search1(){
	...
	$.call("hospital_common.dicthisroutename.search",{data: data},function(rtn){
		if(rtn.routename){
			var html = [];
			var v = rtn.routename;
			for(i=0;i<v.length;i++){
