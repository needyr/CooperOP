				...
				<table class="mytable">
					<thead>
						<tr>
							...
							<th width=100px>诊断编码</th>
							<th width=100px>诊断名称</th>
						</tr>
					</thead>
				</table>
			</div>
...
<script>
	$(function() {
		...
		if ('${param.code}') {
			...
			$.call("hospital_common.dicthisdiagnosis.queryDiagnosisCheck", tj, function(v) {
				if (v) {
					var html = [];
					for (i = 0; i < v.length; i++) {
						html.push('<tr class="is_c" data_id="'+v[i].diagnosis_code+'">');
						html.push('<td width=20px><i class="glyphicon glyphicon-minus remove_one" i_data_id="'+v[i].diagnosis_code+'"></i></td>');
						html.push('<td width=100px>'+v[i].diagnosis_code+'</td>');
						html.push('<td width=100px>'+v[i].diagnosis_name+'</td>');
						html.push('</tr>');
					}
	...
	function search1() {
		...
		$.call("hospital_common.dicthisdiagnosis.queryDiagnosis", tj, function(v) {
			if (v) {
				var html = [];
				for (i = 0; i < v.length; i++) {
					html.push('<tr class="is_c'+v[i].diagnosis_code+'" data_id="'+v[i].diagnosis_code+'">');
					html.push('<td width=20px><i class="glyphicon glyphicon-plus add_one" i_data_id="'+v[i].diagnosis_code+'"></i></td>');
					html.push('<td width=100px>' + v[i].diagnosis_code + '</td>');
					html.push('<td width=100px>' + v[i].diagnosis_name + '</td>');
					html.push('</tr>');
				}
