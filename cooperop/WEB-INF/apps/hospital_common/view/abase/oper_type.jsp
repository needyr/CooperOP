				...
				<table class="mytable">
					<thead>
						<tr>
							...
							<th width=100px>手术等级代码</th>
							<th width=100px>手术等级名称</th>
						</tr>
					</thead>
				</table>
			</div>
...
<script>
	$(function() {
		...
		if ('${param.code}') {
			...
			$.call("hospital_common.dicthisoper.queryOperTypeCheck", tj, function(v) {
				if (v) {
					var html = [];
					for (i = 0; i < v.length; i++) {
						html.push('<tr class="is_c" data_id="'+v[i].operationscale_code+'">');
						html.push('<td width=20px><i class="glyphicon glyphicon-minus remove_one" i_data_id="'+v[i].operationscale_code+'"></i></td>');
						html.push('<td width=100px>' + v[i].operationscale_code + '</td>');
						html.push('<td width=100px>' + v[i].operationscale_name + '</td>');
						html.push('</tr>');
					}
	...
	function search1() {
		...
		$.call("hospital_common.dicthisoper.queryOperType", tj, function(v) {
			if (v) {
				var html = [];
				for (i = 0; i < v.length; i++) {
					html.push('<tr class="is_c'+v[i].operationscale_code+'" data_id="'+v[i].operationscale_code+'">');
					html.push('<td width=20px><i class="glyphicon glyphicon-plus add_one" i_data_id="'+v[i].operationscale_code+'"></i></td>');
					html.push('<td width=100px>' + v[i].operationscale_code + '</td>');
					html.push('<td width=100px>' + v[i].operationscale_name + '</td>');
					html.push('</tr>');
				}
