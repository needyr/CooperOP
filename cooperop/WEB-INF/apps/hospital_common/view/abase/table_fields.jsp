...
<s:page title="选择表字段" disloggedin="false">
<style>
.checked {
...
if('${param.code}'){
	...
	var tj = {};
	tj.code = code.split(',');
	if('${param.table_name}' && product){
		tj.table_name = '${param.table_name}';
		tj.product = product;
		$.call("hospital_common.abase.searchField",tj,function(rtn){
			if(rtn.fields){
				var html = [];
				var v = rtn.fields;
				for(i=0;i<v.length;i++){
					html.push('<tr class="is_c" data_id="'+v[i].field+'">');
					html.push('<td width=20px><i class="glyphicon glyphicon-minus remove_one" i_data_id="'+v[i].field+'"></i></td>');
					html.push('<td width=100px>'+v[i].field+'</td>');
					html.push('<td width=100px>'+(v[i].beiz?v[i].beiz:'')+'</td>');
					html.push('<td width=100px>'+v[i].typename+'</td>');
					html.push('</tr>');
				}
	...
	if('${param.table_name}' && product){
		...
		$.call("hospital_common.abase.searchField",tj,function(rtn){
			if(rtn.fields){
				...
				var v = rtn.fields;
				for(i=0;i<v.length;i++){
					html.push('<tr class="is_c'+v[i].field+'" data_id="'+v[i].field+'">');
					html.push('<td width=20px><i class="glyphicon glyphicon-plus add_one" i_data_id="'+v[i].field+'"></i></td>');
					html.push('<td width=100px>'+v[i].field+'</td>');
					html.push('<td width=100px>'+(v[i].beiz?v[i].beiz:'')+'</td>');
					html.push('<td width=100px>'+v[i].typename+'</td>');
					html.push('</tr>');
