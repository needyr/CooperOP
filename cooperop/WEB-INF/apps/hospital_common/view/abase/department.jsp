...
<s:page title="选择科室" disloggedin="false">
<s:row>
<div class="top">
	<input type="text" name="search" class="search"/>
	<label><input type="checkbox" name="datasouce" value="1"/>住院</label>
	<label><input type="checkbox" name="datasouce" value="2"/>门诊</label>
	<label><input type="checkbox" name="datasouce" value="3"/>急诊</label>
		...
		<table class="mytable">
			<thead>
				<tr>
					...
					<th width=100px>科室编号</th>
					<th width=100px>科室名称</th>
					<th width=100px>科室类型</th>
...
if('${param.datasouce}'){
	var pa = '${param.datasouce}';
	for(var i=0;i<pa.length;i++){
		$('input[name=datasouce][value='+pa[i]+']').attr("checked","true");
	}
}
if('${param.code}'){
	var arr = new Array();
	$("input[name='datasouce']:checked").each(function(i){
	    arr[i] = $(this).val();
	});
	var code = '${param.code}';
	$.call("hospital_common.department.searchCheck",{code: code.split(','),is_permission:('${param.is_permission}'?'${param.is_permission}':'')},function(rtn){
		if(rtn.dept){
			var html = [];
			var v = rtn.dept;
			for(i=0;i<v.length;i++){
			html.push('<tr class="is_c'+v[i].dept_code+'" data_id="'+v[i].dept_code+'">');
			html.push('<td width=20px><i class="glyphicon glyphicon-minus remove_one" i_data_id="'+v[i].dept_code+'"></i></td>');
			html.push('<td width=100px>'+v[i].dept_code+'</td>');
			html.push('<td width=100px>'+v[i].dept_name+'</td>');
			html.push('<td width=100px>'+v[i].outp_or_inp+'</td>');
			html.push('<td width=100px>'+v[i].input_code+'</td>');
			html.push('</tr>');
			}
...
function search1(){
	...
	$.call("hospital_common.department.querySearch",{data: data,datasouce: arr,is_permission:('${param.is_permission}'?'${param.is_permission}':'')},function(rtn){
		if(rtn.dept){
			var html = [];
			var v = rtn.dept;
			for(i=0;i<v.length;i++){
				html.push('<tr class="is_c'+v[i].dept_code+'" data_id="'+v[i].dept_code+'">');
				html.push('<td width=20px><i class="glyphicon glyphicon-plus add_one" i_data_id="'+v[i].dept_code+'"></i></td>');
				html.push('<td width=100px>'+v[i].dept_code+'</td>');
				html.push('<td width=100px>'+v[i].dept_name+'</td>');
				html.push('<td width=100px>'+v[i].outp_or_inp+'</td>');
				html.push('<td width=100px>'+v[i].input_code+'</td>');
