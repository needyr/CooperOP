...
#shuoms{color: #396dff;}
.is_hiddenmore{overflow: hidden;text-overflow: ellipsis;white-space: nowrap;}
</style>
<body>
            ...
            <table border="">
                <c:forEach items="${$return.orders}" var="oo">
                    <tr class="a1 ${empty oo.level?'':'active'}">
					    ...
					    <td style="width: 20px;">${oo.tag}</td>
					    <c:if test="${oo.repeat_indicator eq '0'}"><td style="width: 30px;">临</td></c:if>
					    <c:if test="${oo.repeat_indicator eq '1'}"><td style="width: 30px;">长</td></c:if>
					    <c:if test="${oo.repeat_indicator ne '1' && oo.repeat_indicator ne '0'}"><td  style="width: 50px;">${oo.repeat_indicator}</td></c:if> 
		                ...
		                <td style="width: 180px;" class="is_hiddenmore" title="${oo.drug_message}">${oo.drug_message}</td>
		                <td style="width: 60px;">${oo.doctor}</td>
	                    <c:if test="${oo.sys_order_status eq '0'}"><td style="color: #00a093;width: 60px;">新开医嘱</td></c:if>
					    <c:if test="${oo.sys_order_status ne '0'}"><td style="width: 60px;">在用医嘱</td></c:if>
                    </tr>
                </c:forEach>
            </table>
        </div>
...
$(function(){
	$(".ypsms").on("click", function(){
	    if($(this).attr("data-id")){
			...
			if(drugcode == 'likeup'){drugcode = $(".ypsms").eq(0).attr("data-id")}
			console.log(drugcode);
			layer.open({
				  type: 2,
				  title:'【'+ $(this).text()+'】说明书',
				  area: ['800px', '580px'], //宽高
				  content: "/w/ipc/auditresult/instruction.html?his_drug_code="+drugcode+"#adverse_reaction"
			});
		}
	});
});	
