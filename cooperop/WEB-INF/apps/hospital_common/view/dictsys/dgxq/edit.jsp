...
<s:page title="编辑">
	<s:row>
		<s:form id="dataform">
			...
			<s:row>
				<s:textfield name="dgxq_code" label="病组分组详情代码" value="${dgxq_code}"
					required="true"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield name="dgxq_name" label="病组分组详情名称" value="${dgxq_name}"
					required="true"></s:textfield>
			</s:row>
...
<script type="text/javascript">
	...
	function submitData() {
		...
		/* 判断所需要调用方法 */
		if (upOrAdd == 1) {
			methodName = "hospital_common.dictsys.dgxq.insert";
		} else if (upOrAdd == 2) {
			methodName = "hospital_common.dictsys.dgxq.update";
			fdata.id = '${id}';
		}
		/* 必填验证 */
		if ($("form").valid()) {
