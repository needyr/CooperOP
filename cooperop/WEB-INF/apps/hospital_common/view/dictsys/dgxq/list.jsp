...
<s:page title="分组详情字典">
	<s:row>
		<s:form id="dataform">
			<s:row>
				<s:textfield placeholder="请输入病组分组详情代码或名称" name="name"></s:textfield>
				<s:button label="查询" icon="fa fa-search" onclick="query()" style="background: #368bdb;text-align: center;color: #FFF;"></s:button>
			</s:row>
		</s:form>
	</s:row>
	<s:row>
		<s:table action="hospital_common.dictsys.dgxq.query" autoload="false"
			label="分组详情字典"  id="datatable" sort="true">
			<s:toolbar>
				<s:button label="新增" onclick="editWindow('','新增',1)" icon="fa fa-plus"></s:button>
			</s:toolbar>
			
			<s:table.fields>
				<s:table.field name="dgxq_code" label="病组分组详情代码" sort="true"></s:table.field>
				<s:table.field name="dgxq_name" label="病组分组详情名称" sort="true"></s:table.field>
...
<script type="text/javascript">
	...
	function del(id){
		 $.call("hospital_common.dictsys.dgxq.delete",{"id":id},function(s){
			if(s>0){
				query();	
			}else{
				$.message("删除失败");
