...
<s:page title="编辑">
	<s:row>
		<s:form id="dataform" label="病种分组类型信息">
			<s:toolbar>
			...
			<s:row>			
				<s:textfield  name="dg_type_code" label="分组类型代码" value="${dg_type_code}" required="true"></s:textfield >
			</s:row>
			<s:row>
				<s:textfield  name="dg_type_name" label="分组类型名称" value="${dg_type_name}" required="true"></s:textfield >
			</s:row>
			<s:row>
...
//判断操作是添加还是更新
	function submitData() {
		...
		/* 判断所需要调用方法 */
		if (upOrAdd == 1) {
			methodName = "hospital_common.dictsys.dgtype.insert";
		} else if (upOrAdd == 2) {
			methodName = "hospital_common.dictsys.dgtype.update";
			fdata.id = '${id}';
		}
		/* 必填验证 */
		if ($("form").valid()) {
