...
<s:page title="基金池等级字典">
	<s:row>
		<s:form id="dataform">
			<s:row>
				<s:textfield placeholder="请输入基金池等级代码或名称" name="name"></s:textfield>
	...
	<s:row>
		<s:table action="hospital_common.dictsys.dgplevel.query" autoload="false"
			label="基金池等级字典"  id="datatable" sort="true">
			<s:toolbar>
				<s:button label="新增" onclick="editWindow('','新增',1)" icon="fa fa-plus"></s:button>
			</s:toolbar>
			<s:table.fields>
				<s:table.field name="dg_plevel_code" label="基金池等级代码" sort="true"></s:table.field>
				<s:table.field name="dg_plevel_name" label="基金池等级名称" sort="true"></s:table.field>
...
<script type="text/javascript">
	...
	function del(id){
		$.call("hospital_common.dictsys.dgplevel.delete",{"id":id},function(s){
			if(s>0){
				query();	
			}else{
				$.message("删除失败");
