...
<s:page title="编辑">
	<s:row>
		<s:form id="dataform" label="基金池等级信息">
			<s:toolbar>
			...
			<s:row>			
				<s:textfield  name="dg_plevel_code" label="等级代码" value="${dg_plevel_code}" required="true"></s:textfield >
			</s:row>
			<s:row>
				<s:textfield  name="dg_plevel_name" label="等级名称" value="${dg_plevel_name}" required="true"></s:textfield >
			</s:row>
...
//判断操作是添加还是更新
	function submitData() {
		...
		/* 判断所需要调用方法 */
		if (upOrAdd == 1) {
			methodName = "hospital_common.dictsys.dgplevel.insert";
		} else if (upOrAdd == 2) {
			methodName = "hospital_common.dictsys.dgplevel.update";
			fdata.id = '${id}';
		}
		/* 必填验证 */
		if ($("form").valid()) {
