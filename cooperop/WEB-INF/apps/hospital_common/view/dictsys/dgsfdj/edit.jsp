...
<s:page title="编辑">
	<s:row>
		<s:form id="dataform" label="收费等级信息">
			<s:toolbar>
			...
			<s:row>			
				<s:textfield  name="dgsfdj_code" label="收费等级代码" value="${dgsfdj_code}" required="true"></s:textfield >
			</s:row>
			<s:row>
				<s:textfield  name="dgsfdj_name" label="收费等级名称" value="${dgsfdj_name}" required="true"></s:textfield >
			</s:row>
...
//判断操作是添加还是更新
	function submitData() {
		...
		/* 判断所需要调用方法 */
		if (upOrAdd == 1) {
			methodName = "hospital_common.dictsys.dgsfdj.insert";
		} else if (upOrAdd == 2) {
			methodName = "hospital_common.dictsys.dgsfdj.update";
			fdata.id = '${id}';
		}
		/* 必填验证 */
		if ($("form").valid()) {
