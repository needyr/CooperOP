			...
			<s:row>
				<s:row>
					<c:if test="${not empty param.sys_drug_code}">
						<input type="hidden" name="sys_drug_code"/>
						<s:textfield label="药品编号" name="his_drug_code" value="" cols="1" readonly="true">${sys_drug_code}</s:textfield>
					</c:if>
					<c:if test="${empty param.sys_drug_code}">
						<input type="hidden" name="sys_drug_code"/>
						<s:textfield cols="1" name="his_drug_code" value="" dbl_action='zl_select_dict_his_drug_zdy,dict_his_drug' label="药品" placeholder="点击选择药品"></s:textfield>
					</c:if>
				...
				<s:row>
					<s:richeditor label="生产厂家" name="manufacturer" value="" cols="3" required="true">${manufacturer }</s:richeditor>
				</s:row>
				<s:row>
				...
				<s:row>
					...
					<s:richeditor label="FDA妊娠药物分级" name="fdanotes_id" value="" cols="3">${fdanotes_id }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor  label="用法用量" name="usage_dosage" value="" cols="3">${usage_dosage }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="适应症" name="indications" value="" cols="3">${indications }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="成分" name="ingredients" value="" cols="3">${ingredients }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="功能主治" name="attending" value="" cols="3">${attending }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="不良反应" name="adverse_reaction" value="" cols="3">${adverse_reaction }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="禁忌" name="taboo" value="" cols="3">${taboo }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="注意事项" name="attentions" value="" cols="3">${attentions }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="药物相互作用" name="interaction" value="" cols="3">${interaction }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="特殊人群用药" name="special_crowd" value="" cols="3">${special_crowd }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="药理作用" name="pharmacologic" value="" cols="3">${pharmacologic }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="警告" name="warn" value="" cols="3">${warn }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="孕妇及哺乳期妇女用药" name="drug_pregnant_lactation" value="" cols="3">${drug_pregnant_lactation }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="儿童用药" name="drug_children" value="" cols="3">${drug_children }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="老人用药" name="drug_elderly" value="" cols="3">${drug_elderly }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="用药过量" name="drug_overdose" value="" cols="3">${drug_overdose }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="药理毒理" name="pharmacology_toxicology" value="" cols="3">${pharmacology_toxicology }</s:richeditor>
				</s:row>
				<s:row>
					<s:richeditor label="药代动力学" name="pharmacokinetics" value="" cols="3">${pharmacokinetics }</s:richeditor>
				</s:row>
				<s:row>
				...
				<s:row>
					<s:richeditor label="其余说明" name="other_msg" value="" cols="3">${other_msg }</s:richeditor>
				</s:row>
			</s:row>
		</s:form>
	</s:row>
...
<script type="text/javascript">
	function save(){
		...
		data.sys_drug_code = '${param.sys_drug_code}';
	/* 	console.log(data);
		alert(data.manufacturer);
		return ; */
		if('${param.sys_drug_code}'!=null&&typeof('${param.sys_drug_code}')!='undefind'&&'${param.sys_drug_code}'!=''){
			data.operation_log = '【' + userinfo.name + '】' + '修改';
			$.call("hospital_common.customspecification.update",data,function(s){
	    		$.closeModal(s);
	    	});
		}else{
			$.call("hospital_common.customspecification.isHas",{"sys_drug_code":data.sys_drug_code},function(rtn){
				if(rtn.resultset.length<1){
					data.owner_create=1;
					/* data.sys_drug_code = $("input[name='sys_drug_code']").val(); */
					data.sys_drug_code = $("input[name='his_drug_code']").val();
					data.operation_log = '【' + userinfo.name + '】' + '新增';
	    			$.call("hospital_common.customspecification.insert",data,function(s){
	    	    		$.closeModal(s);
	    	    	});
	    		}else{
