	...
	<s:row>
		<s:form id="form" >
			<s:row>
				...
				<s:textfield name="patientid" label="患者编号/处方号"></s:textfield>
				...
				<s:textfield cols="1"  name="yaoshi_name" label="审方药师"></s:textfield>
			</s:row>
		</s:form>
	</s:row>
	<s:row>
		<s:table id="datatable" label="审方列表"  fitwidth="false" autoload="false" action="hospital_common.showturns.queryAuditAll" sort="true">
            <s:toolbar>
            	<s:button label="刷选" icon="fa fa-search" onclick="query();"></s:button>
            </s:toolbar>
            <s:table.fields>
                <s:table.field name="create_time" label="提交审方时间"></s:table.field>
                ...
                <s:table.field name="state" label="审方结果" datatype="script">
                	...
                	var state_name ;
                	var is_pop = record.is_pop;
                	if(state == 'HL_Y'){
                		state_name = '智能审查通过';
                	}else if(state == 'HL_N'){
                		state_name = '<font style="color: #747f00">智能审拦截,等待医生决策</font>';
                	}else if(state == 'HL_F'){
                		...
                		var flag = record.order_flag;
	                	if(flag == '-1'){
	                		state_name = '智能审查拦截，医生强制用药，药师审查超时，系统返回医生决定，医生确认用药';
	                	}else{
	                	...
	                	var flag = record.order_flag;
	                	if(flag == '-1'){
	                		state_name = '智能审查拦截，医生强制用药，药师审查超时，系统返回医生决定，医生取消用药';
	                	}else{
                		...
                		var flag = record.order_flag;
	                	if(flag == '-1'){
	                		state_name = '<font style="color: #747f00">智能审查拦截，医生强制用药，药师审查超时，系统自动返回医生决定</font>';
	                	}else{
                			state_name = '<font style="color: #747f00">智能审查拦截，药师审查：医生决定，等待医生决策</font>';
	                	}
                	}else if(state == 'DQ'){
                		state_name = '<font style="color: #bd850a">智能审查拦截，医生强制用药，等待药师审查</font>';
                	}else if(state == 'DQ1'){
                		state_name = '智能审查拦截，医生强制用药，药师非工作时间系统自动通过';
                ...
                <s:table.field name="yaoshi_name" label="审方药师" datatype="script">
                ...
                <s:table.field name="cost_time" label="已等待" datatype="script">
                	var state = record.state;
...
<script type="text/javascript">
	...
	function auditDetail(ipc_id){
	  //  $.modal("detailshow.html?id="+ ipc_id,"查看审查详情",{
		  $.modal("/w/ipc/auditflow/show_ywlsb.html?auto_audit_id="+ ipc_id,"查看审查详情",{
	    	width: '900px',
	    	height: '99%',
	        callback : function(e){}
	    });
	...
	function druglist(id, _this){
		$(_this).css('color', '#900987');
		$.modal('/w/ipc/auditresult/druglist.html?auto_audit_id='+ id + '&is_ywlsb=1',"查看医嘱",{
		  	width: '900px',
		  	height: '550px',
		    callback : function(e){}
		});
