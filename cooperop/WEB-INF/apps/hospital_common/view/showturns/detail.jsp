...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/res/hospital_common/css/turns_detail.css" >
...
<script type="text/javascript" src="${pageContext.request.contextPath}/res/hospital_common/bsintf/jquery.min.1.9.1.js"></script>
...
<%--
	ygz.2021.06.30: 该版本的layer.js 与 $.modal() 不兼容，谨慎使用！
	<script src="${pageContext.request.contextPath}/res/hospital_common/bsintf/layer.js" type="text/javascript"></script>
--%>
    		...
    		<c:if test="${not empty $return.hospital_imic.tnum }">
	        	...
	        	<c:forEach items="${$return.hospital_imic.tnum }" var="tmum">
					<li class="nav-content" title="${tmum.sort_name}">
						<a href="#question_${tmum.sort_id}" data-item-id="${tmum.sort_id }">
								${fn:substring(tmum.sort_name, 0, 8)}${fn:length(tmum.sort_name) > 9 ?  '...': ''}
									(<b>${tmum.num }</b>)
						</a>
					</li>
				</c:forEach>
        ...
        <div class="top-div">
            <p class="top-p infobr">
                ...
                <a href="javascript:void(0);" onclick="topatient('${$return.patient.patient_id}','${$return.patient.visit_id}','${$return.patient.doctor_no}');">患者详情</a>
                 ...
                 <c:if test="${not empty $return.hospital_imic.tnum }">
                 	| <a href="javascript:void(0);" onclick="printSF();">打印自费告知书</a>
                 </c:if>
	...
	<div class="box-content">
		...
		<div class="card-tabs-stack graygreen">
			...
			<div id="tab2" style="display: block;">
				...
				<!-- 医保问题明细 -->
				<c:set value="0" var="imic_sort_05" />
				<c:forEach items="${$return.hospital_imic.imic }" var="mr" varStatus="v">
				    ...
				    <!--         <p id="dorctor_comment">
				             ...
				             <c:if test="${mr.sort_id eq '05'}">
				             	<c:set value="1" var="imic_sort_05" ></c:set>
				             </c:if>
										...
										<c:choose>
											<c:when test="${imic_can_toselfpaid eq 1}">
												<label for="is_use${v.index}">
								             		...
								             		<%-- <span class="imic_zzf_desc">患者自愿自费使用[同一药品仅操作一次即可，若[返回调整]，则此操作无效</span> --%>
								             		<span class="imic_zzf_desc">将本次使用药品转为自费</span>
								             	</label>
											</c:when>
											<c:otherwise>
												若确认该问题且必须使用，可将该项目转为自费使用（HIS系统中操作转自费）
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>
				...
				<c:forEach items="${$return.ipc.item_info }" var="iteminfo" varStatus="st">
					...
					<div class="boc-list">
						...
						<c:if test="${iteminfo.check_result_state eq 'B' }">
				            ...
				            <p>医嘱：<b>
				            	<c:set var="check_sj" value="1"></c:set>
					            <c:forEach items="${$return.ipc.orders }" var="ord">
					            	<c:if test="${iteminfo.sort_name eq 'TPN'}">
					            		<c:if test="${check_sj eq '2'}">
					            			&emsp;&emsp;&emsp;
					            		</c:if>
				            			<c:set var="check_sj" value="2"></c:set>
					            	</c:if>
    	...
    	<div class="content" id="qadivce" style="display:none;">
            <div class="main">
            	...
            	<c:if test="${pa.is_null eq '1'}">
	            	<c:forEach items="${pa.tnum }" var="item" varStatus="status">
	            	<c:set var="q_num" value="-1"></c:set>
	            	<c:if test="${item.check_result_state eq 'N' or item.check_result_state eq 'Q'}">
	            	<c:set var="q_num" value="${q_num + 1 }"></c:set>
	           		<div class="list">
	                	<p class="title" id="titles" data-product="${pa.product_code}">${item.sort_name }</p>
	                    	...
	                    	<c:if test="${av.apa_check_sorts_name == item.sort_name}">
	                        	<c:choose>
	                            	<c:when test="${av.is_must == 1}">
	                            		<div style="display:none" class="advice_is_must${q_num}${pa.product_code}">1</div>
	                            		<c:if test="${not empty av.detail_id}">
...
<script type="text/javascript">
 ...
 var patient = {};
 patient.patient_id = '${$return.patient.patient_id}';
 patient.visit_id = '${$return.patient.visit_id}';
 ...
 patient.doctor_no = '${$return.patient.doctor_no}';
 patient.imic_sort_05 = '${imic_sort_05}';
...
/*  var ipc_state = '${$return.ipc.state}';
 ...
 var audit_d_type = '${$return.d_type}';
 var id1;
$(function(){
	//工作站是否能进行转自费标志 0表示关闭，1表示开启
	var dtype_can_toselfpaid = ${$return.dtype_can_toselfpaid};
	authNotice();
	report_text();//report提示
	<c:if test="${not empty $return.hospital_imic}">
		imic_zzf(dtype_can_toselfpaid);//限专药品转自费操作
		check_print();
	</c:if>
 ...
 function authNotice(){
	 if("${empty $return.appinfo? 0: 1}" == "1"){
		 var msg = '告知：您的服务即将过期，请联系管理员续约！';
		 layer.msg(msg + "售后电话：028-62378387，邮箱：123456@163.com", {time: 5000, icon:7, offset: 'lb'});
	 }
 }
...
function qzqd() {
	...
	if(rtnflag.ipc && rtnflag.ipc == 'HL_N'){
		...
		//++++增加,重复提交程序对比判断
		is_cftj('${$return.ipc.ipc_auto_id}','${$return.patient.patient_id}','${$return.patient.visit_id}');
		if(rep_check == 1){
			return
		}
		//--------------------------------
