...
<s:page title="合理用药审查部门维护" >
	...
	<s:row>
		<s:table id="datatable" label="参与合理用药审查的部门列表" autoload="false" action="hospital_common.auditset.checkdept.query"  >
			<s:toolbar>
			...
			<s:table.fields>
				...
				<s:table.field name="dept_name" label="部门" ></s:table.field>
...
function _open(dept){
	$.call('hospital_common.auditset.checkdept.open_close',{type:'open',dept:dept,product_code:'ipc'},function(){
		query();
	})
}

function _close(dept){
	$.call('hospital_common.auditset.checkdept.open_close',{type:'close',dept:dept,product_code:'ipc'},function(){
		query();
	})
}
</script>