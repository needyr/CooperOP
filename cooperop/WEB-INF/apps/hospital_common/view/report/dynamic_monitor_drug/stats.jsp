...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">全院药品动态监测排名表</span>
	  ...
	  var ins1 = table.render({
	    ...
	    ,done: function(res, curr, count){
	    	$("#export").off('click').click(function(){
	    		tabletoexcel(ins1.config.cols, real_re, '', ins1.config.title)
...
function query1(){
	...
	$.call('hospital_common.report.dynamic_monitor_drug.query_stats',data,function(rtn){
		var real_re = [];
			...
			for(var i=0;i<res.length;i++){
				...
				if(_re.msg){
					...
					for(var j = 0;j<msg.length;j++){
						...
						_re['rowid_money_'+msg_json.date] = msg_json.rowid_money+'  ('+msg_json.money+')';
						if(j != 0){
							_re['rowid_fdl_'+msg_json.date] = msg_json.rowid_fdl+'  ('+msg_json.fdl+')';
						}
					}
				}
				if(_re.is_warn){
		...
		data_cols1.push({field:'shengccj', title:'生产厂家', rowspan: 2});
		if(cxtj.time_unit == '0'){
			for(var i = 0;i<mons.length;i++){
				data_cols1.push({field:'hbl_'+i, title: mons[i], colspan: 2, align:'center'});
				data_cols2.push({field: 'rowid_money_'+mons[i], title:'金额排名(金额)'});
				data_cols2.push({field: 'rowid_fdl_'+mons[i], title:'浮动率排名(浮动率)'});
			}
		}else{
			for(var i = 0;i<mons.length;i++){
				data_cols1.push({field:'hbl_'+i, title: mons[i], colspan: 2, align:'center'});
