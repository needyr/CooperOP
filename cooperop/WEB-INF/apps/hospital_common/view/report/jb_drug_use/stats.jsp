...
<s:form id="form" collapsed="false" extendable="true">
	...
	<s:row>
		...
		<s:radio label="统计对象" cols="2" name="obj" value="">
			<s:option value ="" label="所有病人"></s:option>
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">全院基本药物使用情况统计表</span>
...
function create_table1(real_re){
	cols1 = [[
	    ...
	    ,{field:'patient_no', title:'住院号'}
	    ,{field:'patient_name', title:'病人姓名',templet:function(record){
	    ...
	    ,{field:'drug_kind_num', title:'药品品种数(种)'}
	    ,{field:'jb_drug_kind_num', title:'基本药物品种数(种)'}
	    ,{field:'jb_drug_kind_num_drug_kind_num', title:'基本药物品种数所占比例(%)',templet: function(record){
	    	  var val = (Number(record.jb_drug_kind_num)/Number(record.drug_kind_num)*100).toFixed(2);
	    	  return isNaN(val)?'':val;
	    }}
	    ,{field:'drug_money', title:'药品金额(元)'}
	    ,{field:'jb_drug_money', title:'基本药物金额(元)'}
	    ,{field:'jb_drug_money_drug_money', title:'基本药物金额所占比例(%)',templet: function(record){
	    	var val = (Number(record.jb_drug_money)/Number(record.drug_money)*100).toFixed(2);
	    	return isNaN(val)?'':val;
	    }}
    ]];
...
function query1(){
	...
	$.call('hospital_common.report.jb_drug_use.query_stats',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			var res = rtn.resultset;
			var drug_kind_num_zong = 0;
			var jb_drug_kind_num_zong = 0;
			var drug_money_zong = 0;
			var jb_drug_money_zong = 0;
			for(var i=0;i<res.length;i++){
				res[i].rowid = i + 1;
				drug_kind_num_zong = drug_kind_num_zong + Number(res[i].drug_kind_num);
				jb_drug_kind_num_zong = jb_drug_kind_num_zong + Number(res[i].jb_drug_kind_num);
				drug_money_zong = drug_money_zong + Number(res[i].drug_money);
				real_re.push(res[i]);
				if(res.length - 1 == i){
					real_re.push({'rowid':'总计',
						'drug_kind_num':'A='+drug_kind_num_zong,
						'jb_drug_kind_num':'B='+(isNaN(jb_drug_kind_num_zong)?'':jb_drug_kind_num_zong),
						'drug_money':'D='+drug_money_zong.toFixed(2),
						'jb_drug_money':'E='+jb_drug_money_zong,
						});
					var val_zong1 = (jb_drug_kind_num_zong/drug_kind_num_zong*100).toFixed(2);
					var val_zong2 = (jb_drug_money_zong/drug_money_zong*100).toFixed(2);
					real_re.push({'rowid':'%',
						'jb_drug_kind_num':'G='+(res[i].jb_patient_num/rtn.count*100).toFixed(2),
						'drug_money':'C='+ (isNaN(val_zong1)?'':val_zong1),
						'jb_drug_money_drug_money':'F='+ (isNaN(val_zong2)?'':val_zong2),
						});
