...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">科室基本药物使用情况统计表</span>
...
function create_table1(real_re){
	cols1 = [[
		{field:'rowid',width:80, title:'序号',totalRowText:'合计'}
	    ,{field:'dept_name', title:'科室'}
	    ,{field:'drug_kind_num', title:'药品品种数(种)', totalRow:true}
	    ,{field:'jb_drug_kind_num', title:'基本药物品种数(种)', totalRow:true}
	    ...
	    ,{field:'drug_money', title:'药品金额(元)', totalRow:true,templet: function(record){
	    	var val = Number(record.drug_money).toFixed(2)
	    	return isNaN(val)?'':val;
	    }}
	    ,{field:'jb_drug_money', title:'基本药物金额(元)', totalRow:true}
	    ,{field:'jb_drug_money_drug_money', sort:true, title:'基本药物金额所占比例(%)',templet: function(record){
	    	var val = (Number(record.jb_drug_money)/Number(record.drug_money)*100).toFixed(2);
	    	return isNaN(val)?'':val;
	    }}
	    ,{field:'jb_patient_num', title:'使用基本药物患者例数(名)'}
	    ,{field:'patient_num', title:'科室总治疗患者例数(名)'}
	    ,{field:'jb_patient_num_patient_num', title:'使用基本药物患者数所占比例(%)',templet: function(record){
	    	var val = (Number(record.jb_patient_num)/Number(record.patient_num)*100).toFixed(2);
	    	return isNaN(val)?'':val;
	    }}
    ]];
...
function query1(){
	...
	$.call('hospital_common.report.jb_drug_use.query_dept',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
