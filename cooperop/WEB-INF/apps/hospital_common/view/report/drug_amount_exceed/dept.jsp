...
<s:page title="科室门(急)诊处方药品金额超过N元清单">
<!-- <div class="top"> -->
<s:form id="form" collapsed="false" extendable="true">
	...
	<s:row>
		<s:textfield cols="2" label="处方药品金额≥" name="range" number="true" required="true"></s:textfield>
	</s:row>
</s:form>
<!-- </div> -->
<div class="bottom">
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">科室门(急)诊处方药品金额超过N元清单</span>
...
function create_table1(real_re){
	cols1 = [[
	    ...
	    ,{field:'zong_count', title:'科室处方总数(张)',sort:true}
	    ,{field:'range_count', title:'科室药品金额超过'+(cxtj.range?cxtj.range:0)+'元的处方数量(张)',sort:true}
	    ,{field:'range_count_zong_count', title:'占科室处方总数比例(%)',sort:true,templet: function(record){
...
function query1(){
	...
	$.call('hospital_common.report.drug_amount_exceed.query_dept',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			var range_zong = 0;
			...
			for(var i=0;i<res.length;i++){
				...
				if(res.length - 1 == i){
					...
					real_re.push({'dept_name':'处方药品金额：≥'+ cxtj.range + '元的处方总数：'+range_zong+'张'});
				}
			}
		}
