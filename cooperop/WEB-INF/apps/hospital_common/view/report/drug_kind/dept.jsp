...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">科室药品品种数统计表</span>
...
function query1(){
	...
	$.call('hospital_common.report.drug_kind.query_dept',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			...
			for(var i=0;i<res.length;i++){
				...
				if(res.length - 1 == i){
					...
					if(is_null("jixing")){
						real_re.push({'dept_name':'剂型：'+cxtj.jixing_name});
					}
					if(is_null("dept_name")){
						real_re.push({'dept_name':'科室：'+cxtj.dept_name});
					}
					if(is_null("doctor_name")){
						real_re.push({'dept_name':'医生：'+cxtj.doctor_name});
					}
					if(cxtj.yllb_code){
						real_re.push({'dept_name':'药理类别：'+cxtj.yllb_name});
					}
				}
			}
		}
