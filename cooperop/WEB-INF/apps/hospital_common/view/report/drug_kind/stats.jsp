...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">全院药品品种数统计表</span>
...
function query1(){
	...
	$.call('hospital_common.report.drug_kind.query_stats',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			...
			for(var i=0;i<res.length;i++){
				var data_res = res[i];
				var map_res = {};
				map_res.xm1 = '药品总品种数(种)';
				map_res.res1 = data_res.zong;
				map_res.xm2 = '非限制抗菌药物品种数(种)';
				map_res.res2 = data_res.kjy1;
				real_re.push(map_res);

				map_res = {};
				map_res.xm1 = '基本药物品种数(种)';
				map_res.res1 = data_res.jbyw;
				map_res.xm2 = '限制抗菌药物品种数(种)';
				map_res.res2 = data_res.kjy2;
				real_re.push(map_res);

				map_res = {};
				map_res.xm1 = '抗菌药物品种数(种)';
				map_res.res1 = data_res.kjy;
				map_res.xm2 = '特殊抗菌药物品种数(种)';
				map_res.res2 = data_res.kjy3;
				real_re.push(map_res);

				map_res = {};
				map_res.xm1 = '抗结核药物品种数(种)';
				map_res.res1 = data_res.kjy_kjh;
				map_res.xm2 = '';
				map_res.res2 = '';
				real_re.push(map_res);

				if(res.length - 1 == i){
					real_re.push({'xm1':'1.统计范围'});
					real_re.push({'xm1':'时间：'+cxtj.start_time+'~'+cxtj.end_time});
					real_re.push({'xm1':'类型：'+ (cxtj.d_type && cxtj.d_type.length >0?cxtj.d_type_name:'全部')});
					if(is_null("jixing")){
						real_re.push({'xm1':'剂型：'+cxtj.jixing_name});
					}
					if(is_null("dept_name")){
						real_re.push({'xm1':'科室：'+cxtj.dept_name});
					}
					if(is_null("doctor_name")){
						real_re.push({'xm1':'医生：'+cxtj.doctor_name});
					}
					if(cxtj.yllb_code){
						real_re.push({'xm1':'药理类别：'+cxtj.yllb_name});
					}
				}
			}
		}
		var data_cols1 = [];
		data_cols1.push({field:'xm1', title:'统计项目'});
		data_cols1.push({field:'res1', title:'结果'});
		data_cols1.push({field:'xm2', title:'统计项目'});
		data_cols1.push({field:'res2', title:'结果'});
		cols1 = [data_cols1];
