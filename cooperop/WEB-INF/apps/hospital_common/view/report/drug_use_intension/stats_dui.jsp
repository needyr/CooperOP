...
var cols1 = [[
    ...
    ,{field:'intension', title:'使用强度', width:80,templet: function(record){
  	  return (Number(record.intension) || 0).toFixed(2) ;
    }}
    ,{field:'_data1', title:'占药物类总额比例(%)', width:200,templet: function(record){
        return (Number(record.kind_drug_money)/Number(record.all_drug_money)*100 || 0).toFixed(2);
    }}
...
function query1(){
	...
	$.call('hospital_common.report.drug_use_intension.query_dui',data,function(rtn){
		var real_re = [];
		var all_money = 0;
		var sylc = 0;
		var ddds = 0;
