...
<s:page title="医生药品消耗情况及使用量(DDDs)统计表">
<s:form id="form" collapsed="false" extendable="true">
	...
	<s:row>
		<s:radio label="排名方式" cols="2" name="pm" value="1">
			<s:option value ="1" label="按医生药品金额排序"></s:option>
			<s:option value ="2" label="按药品医生金额排序"></s:option>
			<s:option value ="3" label="按医生药品DDDs排序"></s:option>
			<s:option value ="4" label="按药品医生DDDs排序"></s:option>
		</s:radio>
	</s:row>
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">医生药品消耗情况及使用量(DDDs)统计表</span>
...
var cols1 = [[
	{field:'doctor_name', title:'医生名称', width:200}
	,{field:'doctor_no', title:'医生编码', width:80}
	,{field:'doc_dept_name', title:'医生所属科室名称', width:120}
	...
	layui.use('table', function(){
	  ...
	  var ins1 = table.render({
	    ...
	    ,done : function(res, curr, count) {
	        merge(res,['doctor_name','doctor_no','doc_dept_name'],[0,1,2]);
...
function query1(){
	...
	cxtj = data;
	$.call('hospital_common.report.drug_use_intension.query_expend_doctor_dui',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			...
			for(var i=0;i<res.length;i++){
				...
				res[i].doctor_name = res[i].doctor_name+(cxtj.pm=='3' || cxtj.pm=='4'?("累计DDDs="+res[i].ddds_zong):("总金额="+res[i].all_money_zong));
				real_re.push(res[i]);
				if(res.length - 1 == i){
