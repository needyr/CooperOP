...
<s:form id="form" collapsed="false" extendable="true">
	...
	<s:textfield label="医生前几名" cols="1" number="true" name="dept_pm" required="true"></s:textfield>
	...
	<s:row>
		<s:radio label="排名方式"  value="drug_money"  cols="1" name="pmfs">
			<s:option label="金额" value="drug_money"></s:option>
			<s:option label="数量" value="drug_num"></s:option>
		</s:radio>
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">医生药品使用金额及数量排名表</span>
...
function create_table1(real_re){
	if(real_re.length <= 0){
		var xs_pmfs = ($("#form").getData().pmfs == 'drug_money'?'金额':'数量');
		cols1 = [[
		    {field:'rowid', title:'排名('+xs_pmfs+')', width:100}
		    ,{field:'drug_name', title:'药品名称'}
		    ,{field:'jixing', title:'剂型'}
		    ,{field:'druggg', title:'规格'}
		    ,{field:'drug_num', title:'数量'}
		    ,{field:'money', title:'总金额(元)'}
	    ]];
	}
...
function query1(){
	...
	$.call('hospital_common.report.drug_amount.query_doctor',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			var res = rtn.resultset;
			for(var i=0;i<res.length;i++){
				var data_res = res[i];
				if(data_res.doctor_rank){
					var doctor_ranks = data_res.doctor_rank.split(',');
					for(var j=1;j<=doctor_ranks.length;j++){
						data_res['kz_'+j] = doctor_ranks[j-1].split(':')[1];
					}
				}
				...
				if(res.length - 1 == i){
					...
					real_re.push({'rowid':'统计名次：药品前'+cxtj.drug_pm+'名,医生前'+cxtj.dept_pm+'名'});
		...
		var xs_pmfs = (cxtj.pmfs == 'drug_money'?'金额':'数量');
		var data_cols1 = [];
		var data_cols2 = [];
		data_cols1.push({field:'rowid', title:'排名('+xs_pmfs+')', width:100, rowspan: 2});
		...
		data_cols1.push({field:'drug_num', title:'数量', rowspan: 2});
		data_cols1.push({field:'money', title:'总金额(元)', rowspan: 2});
		data_cols1.push({field:'', title:'医生排名', colspan: +cxtj.dept_pm, align:'center'});
		for(var i = 1;i <= +cxtj.dept_pm;i++){
			data_cols2.push({field:'kz_'+i, title:'医生'+i+'('+xs_pmfs+')'});
		}
