...
<s:form id="form" collapsed="false" extendable="true">
	...
	<s:row>
	<s:textfield label="药品前几名" cols="2" number="true" name="drug_pm" required="true"></s:textfield>
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <span style="font-size:14px">全院药品使用金额及数量排名表</span>
...
function create_table1(real_re){
	cols1 = [[
	    {field:'rowid', title:'排名('+($("#form").getData().pmfs == 'drug_money'?'金额':'数量')+')', width:100}
...
function query1(){
	...
	$.call('hospital_common.report.drug_amount.query_stats',data,function(rtn){
		var real_re = [];
		if(rtn.count>0){
			...
			for(var i=0;i<res.length;i++){
				...
				if(res.length - 1 == i){
					...
					real_re.push({'rowid':'统计名次：药品前'+cxtj.drug_pm+'名'});
