...
function create_table1(params){
	var data_cols1 = [];
	var data_cols2 = [];
	data_cols1.push({field:'patient_no', title:'医嘱号', width:80, rowspan: 2});
	...
	data_cols1.push({field:'out_diagnosis', title:'出院诊断', width:200, rowspan: 2});
	...
	data_cols1.push({field:'is_lhyy', title:'联合用药', width:80, rowspan: 2,template:function(record){
			var str = "";
	...
	data_cols1.push({field:'cfxx', title:'处方信息', colspan: 10, align:'center'});
	...
	data_cols2.push({field: 'start_date_time', title:'开始时间', width:120});
	data_cols2.push({field: 'stop_date_time', title:'结束时间', width:120});
	data_cols2.push({field: 'yyts', title:'用药天数', width:100});
	...
	layui.use('table', function(){
	  ...
	  var ins1 = table.render({
	    elem: '#test'
	    ,url:'/w/hospital_common/report/kjy/querykjyoutpuse.json'
	    ,where: {'json':JSON.stringify(params)}
	    ,toolbar: '#toolbarDemo'
	    ,title: '出院患者抗菌药物使用情况调查表'
	    ...
	    ,done: function(res, curr, count){
	    	merge(res.resultset, curr, count,['patient_no','patient_id','patient_name',
	    	...
	    	$("#export").off('click').click(function(){
	    		$.call('hospital_common.report.kjy.querykjyoutpuse',{'json':JSON.stringify(params),limit:$('[lay-id=test] .layui-laypage-limits select').val(),layui_page:$('[lay-id=test] .layui-laypage-em').next().text()},function(myresult){
