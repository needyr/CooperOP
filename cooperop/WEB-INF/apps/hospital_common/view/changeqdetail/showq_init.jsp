...
<s:page title="查看审查问题" disloggedin="true">
    ...
    <s:row>
        <s:form id="form" fclass="portlet light bordered" collapsed="true" extendable="true">
            ...
            <s:row>
                ...
                <s:textfield label="科室名称" name="dept_name" cols="1"
                             dbl_action='zl_select_jcx057_2_01_common,jcx057_2_common'
            ...
            <s:row>
                ...
                <s:textfield label="问题类型" name="sort_name" cols="1"
                             dbl_action='zl_select_jcx057_2_02_common,jcx057_2_common'
                             disabled="disabled"></s:textfield>
                <s:textfield label="医生名称" name="doctor" cols="1" placeholder="多个医生用英文逗号隔开"></s:textfield>
            </s:row>
            <s:row>
    ...
    <s:row>
        <s:table select="single" id="datatable" fitwidth="false" label="审查问题" autoload="false"
                 action="hospital_common.changeqdetail.query_init" sort="true">
            <s:toolbar>
                <s:button label="筛选列 " icon="fa fa-random" onClick="shaixuan();"></s:button>
                <%-- <s:button label="导出2" icon="fa fa-random" action="toexcel"></s:button> --%>
                <s:button label="导出" icon="fa fa-random" onclick="daochu()"></s:button>
                <s:button label="查询" onclick="query()" icon="fa fa-search"></s:button>
            </s:toolbar>
            <s:table.fields>
                <s:table.field name="patient_id" label="患者ID" width="120px" sort="true"></s:table.field>
                <s:table.field name="kdlx" label="开单类型" width="120px" sort="true" datatype="script">
                    return (record.d_type == 1 ? '住院': record.d_type == 2 ? '门诊':'急诊') + (record.p_type == 1?'医嘱':'处方');
                </s:table.field>
                <s:table.field name="patient_name" label="患者姓名" datatype="script" width="120px" sort="true">
                    var html = [];
                    if(record.patient_name){
                    var ss = record.patient_name.replace(/<br>/g, '^').split('^')[0].split(' ');
                ...
                <s:table.field name="sex" label="性别" datatype="script" width="80px" sort="true">
                    var html = [];
                    if(record.patient_name){
                    var ss = record.patient_name.replace(/<br>/g, '^').split('^')[0].split(' ');
                    html.push(ss[1]);
                ...
                <s:table.field name="age" label="年龄" datatype="script" width="80px" sort="true">
                    var html = [];
                    if(record.patient_name){
                    var ss = record.patient_name.replace(/<br>/g, '^').split('^')[0].split(' ');
                    html.push(ss[2]);
                ...
                <s:table.field name="doctor" label="医生" width="130px" sort="true"></s:table.field>
                ...
                <s:table.field name="ordermessage" label="医嘱信息(药品名|用法|用量|频次)" width="580px" datatype="script"
                               sort="true">
                    var html = [], code = [] ,drug = [];
                    ...
                    drug = record.ordermessage.split(',');
                    }
                    if(drug){
                    var _code = '';
                    html.push('<table border="1" style="border-top: 1px solid;border-color: black !important" cellspacing="0" cellpadding="0" width="100%">');
                    for(var i = 0; i< drug.length; i++){
                    var drug_dd = drug[i].split('[')[0].split(' ');
                    var drug_name = ''
                    var drug_tl = ''
                    if(drug[i].split('[')[1]){
                    drug_name = drug[i].split('[')[1].split(' ')[0];
                    drug_tl = drug[i].split('[')[1].split(' ')[2]?drug[i].split('[')[1].split(' ')[2].replace(']',''):'';
                    }
                    if(code[i]){
                    _code = code[i].replace('[', '');
                    _code = code[i].replace(']', '');
                    html.push('<tr><td width="260px"><a onclick="drugSms(\''+_code+'\');" data-id = "'+drug+'">'+drug_name+ '</a></td>');
                    html.push('<td width="100px">'+drug_dd[0]+'</td>');
                    html.push('<td width="100px">'+drug_tl+'</td>');
                    html.push('<td width="100px">'+drug_dd[1]+'</td></tr>');
                    }else{
                    html.push('<tr><td width="260px">'+drug_name+ '</td>');
                    html.push('<td width="100px">'+drug_dd[0]+'</td>');
                    html.push('<td width="100px">'+drug_tl+'</td>');
                    html.push('<td width="100px">'+drug_dd[1]+'</td></tr>');
                    }
                ...
                <s:table.field name="description" label="描述" width="400px" maxlength="10" datatype="script" sort="true">
                    ...
                    return '<span title="'+description+'">'+'<span style="display: block;width:400px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">'+description+'</span>'+'<a onclick="copy(\''+str+'\');"><i class="fa fa-copy" title="复制"></i></a></span>';
                    }else{
                    return ' ';
                    }
                ...
                <s:table.field name="diagnosis_name" label="诊断" sort="true" width="280px"></s:table.field>
                ...
                <s:table.field name="shenc_pass_pharmacist_advice" label="调整人意见" sort="true" width="200px"
                               datatype="script">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('.dataTables_scrollBody').css('height', $(window).height() -
            290 - $("div[ctype='form']").parent().parent().parent().height());
    ...
    function query() {
        //debugger
        var formData = $('#form').getData();
        //$("#datatable").parents('.portlet').children('.portlet-title').children('.caption').text("审查结果"+formData.s_time+'_'+formData.e_time)
                ...
                callback: function (rtn) {
                    if (rtn) {
                        ...
                        if (rtn.pharmacist_todoctor_advice) {
                            $.call("hospital_common.changeqdetail.updateTmp", {
                                cz: "0",
    ...
    function init() {
        ...
        }, {async: true, remark: false, timeout: null});
    }
