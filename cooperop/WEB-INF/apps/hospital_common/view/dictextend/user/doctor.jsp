...
<s:page title="选择医生" disloggedin="false">
<s:row>
<div class="top">
...
if('${param.drugtagbh}'){
	...
	$.call("hospital_common.dictextend.user.searchByUserTag",{drugtagbh: code},function(rtn){
		if(rtn.doctor){
			var html = [];
			var v = rtn.doctor;
			for(i=0;i<v.length;i++){
				html.push('<tr class="is_c" data_id="'+v[i].no+'">');
...
function search1(){
	...
	$.call("hospital_common.user.search",{data: data},function(rtn){
		if(rtn.doctor){
			var html = [];
			var v = rtn.doctor;
			for(i=0;i<v.length;i++){
			...
			$('#start_choose tbody tr').each(function(e){
				var data ='#showdrug_table [data_id='+'"'+$(this).find('td').eq(0).text()+'"'+']';
...
function addAll(){
	...
	$.call('hospital_common.dictextend.user.updateTag',{user_id: all_code,drugtagbh:'${param.drugtagbh}'},function(){
		$.closeModal(true);
	})
}
