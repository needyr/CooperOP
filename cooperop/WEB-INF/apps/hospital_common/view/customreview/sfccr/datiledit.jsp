	...
	<s:row>
		<s:form id="form" label="肌酐清除率">
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_shengfccr_06,shengfangzl_ccr' disabled="disabled"></s:textfield>
			...
			<s:row>
				<s:textfield label="使用频率(N次/日)" name="frequency_n"  cols="3" value="${frequency_n}" dbl_action='zl_select_shengfccr_07,shengfangzl_ccr' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<input type="hidden" name="xmbh" value="${xmbh}" />
				<s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}" dbl_action='zl_select_shengfccr_01,shengfangzl_ccr' disabled="disabled"></s:textfield>
				<%-- <s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}"></s:textfield> --%>
				<s:textfield label="公式" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengfccr_02,shengfangzl_ccr' disabled="disabled"></s:textfield>
				<s:textfield label="值" name="value"  cols="1" value="${value}"     dbl_action='zl_select_shengfccr_05,shengfangzl_ccr' ></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.customreview.sfccr.save",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
