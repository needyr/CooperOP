...
<s:page title="编辑频率问题" ismodal="true">
	<s:row>
		<s:form id="form" label="频率问题">
			<s:toolbar>
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_shengfpl_06,shengfangzl_pl' disabled="disabled"></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename"  cols="2" value="${routename}" dbl_action='zl_select_shengfpl_07,shengfangzl_pl' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<input type="hidden" name="xmbh" value="${xmbh}" />
				<s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}" dbl_action='zl_select_shengfpl_01,shengfangzl_pl' disabled="disabled"></s:textfield>
				<%-- <s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}" disabled="disabled"></s:textfield> --%>
				<s:textfield label="公式" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengfpl_02,shengfangzl_pl' disabled="disabled"></s:textfield>
				<s:textfield label="值" name="value"  cols="1" value="${value}"     dbl_action='zl_select_shengfpl_05,shengfangzl_pl' ></s:textfield>
			</s:row>
			<%-- <s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.customreview.sfpl.save",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "修改频率问题"}, function(){});
			}else{
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "新增频率问题"}, function(){});
			}
	     }
	}
