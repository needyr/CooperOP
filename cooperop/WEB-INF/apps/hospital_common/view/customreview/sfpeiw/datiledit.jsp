...
<s:page title="编辑配伍问题" ismodal="true">
	<s:row>
		<s:form id="form" label="配伍问题">
			...
			<s:row>
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_shengfpeiw6,shengfangzl_peiw' disabled="disabled"></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename" value="${routename}"  cols="2"  dbl_action='zl_select_shengfpeiw7,shengfangzl_peiw' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<input type="hidden" name="xmbh" value="${xmbh}" />
				<s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}" dbl_action='zl_select_shengfpeiw2,shengfangzl_peiw' disabled="disabled"></s:textfield>
				<%-- <s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield> --%>
				<s:textfield label="公式" disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengfpeiw3,shengfangzl_peiw'></s:textfield>
				<s:textfield label="值" name="value"  cols="1" value="${value}"    dbl_action='zl_select_shengfpeiw1,shengfangzl_peiw' ></s:textfield>
			</s:row>
			<%-- <s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			sdata.id='${param.id}';
			$.call("hospital_common.customreview.sfpeiw.save",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
