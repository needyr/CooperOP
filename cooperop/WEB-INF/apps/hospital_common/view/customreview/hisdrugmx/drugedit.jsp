...
<s:page title="修改药品属性" >
	<s:row>
		<s:form id="form" label="编辑药品属性">
			...
			<s:row>
				<s:textfield label="项目名称" name="xiangm"  cols="4" value="${xiangm}" dbl_action='zl_select_dict_his_drug_mx_01,dict_his_drug_mx' ></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="4" value="${value}" dbl_action='zl_select_dict_his_drug_mx_02,dict_his_drug_mx' ></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			var xmid = '${param.xmid}';
			if(xmid){
				$.call("hospital_common.customreview.hisdrugmx.update",tdata,function(rtn){
					$.closeModal(rtn);
				});
				...
				$.call("hospital_common.customreview.hisdrugmx.insert",tdata,function(rtn){
					$.closeModal(rtn);
				});
				$.call("hospital_common.dictdrug.updateCz", 
