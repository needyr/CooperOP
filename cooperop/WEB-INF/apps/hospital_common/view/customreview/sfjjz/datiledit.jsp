	...
	<s:row>
		<s:form id="form" label="禁忌症问题">
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_shengfjjz_06,shengfangzl_jjz' disabled="disabled"></s:textfield>
		...
		<s:table id="datatable" label="诊断信息" autoload="false" action="hospital_common.customreview.sfjjz.query_mx" sort="true"  limit="10">
			<s:toolbar>
...
$(function(){
	...
	function save(){
		...
		$.call("hospital_common.customreview.sfjjz.save",sdata,function(s){
			$.closeModal(s);
    	});
		if(sdata.id){
  ...
  //打开药品说明书
	...
	function Delete(id,wt){
		$.confirm("是否确认删除，删除后无法恢复！",function callback(e){
			if(e==true){
				$.call("hospital_common.customreview.sfjjz.delete_mx",{"id":id},function(e){
					query();
					$.call("hospital_common.dictdrug.updateCz", 
	...
	function add(){
		$.modal("/w/hospital_common/customreview/sfjjz/diagnosis.html","新增",{
			width:"60%",
			height:"70%",
			parent_id: '${param.id}',
			spbh: '${param.spbh}',
	...
	function adds(){
		$.modal("/w/hospital_common/customreview/sfjjz/add_pl.html","新增",{
			width:"60%",
			height:"70%",
			parent_id: '${param.id}',
			spbh: '${param.spbh}',
	...
	function add_check(){
		$.modal("/w/hospital_common/customreview/sfjjz/add_check.html","新增",{
			width:"60%",
			height:"70%",
			parent_id: '${param.id}',
			spbh: '${param.spbh}',
	...
	function update(id){
		$.modal("/w/hospital_common/customreview/sfjjz/diagnosis.html","修改",{
			width:"60%",
			height:"70%",
			id: id,
			parent_id: '${param.id}',
