...
<s:page title="编辑孕妇用量问题" ismodal="true">
	<s:row>
		<s:form id="form" label="孕妇用量问题">
			<s:toolbar>
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" disabled="disabled" value="${nianl_c}" dbl_action='zl_select_shengfyl_child_05,shengfangzl_yongl_child'></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename"  cols="2" disabled="disabled" value="${routename}" dbl_action='zl_select_shengfyl_child_07,shengfangzl_yongl_child' ></s:textfield>
			</s:row>
			<s:row>
				...
				<s:autocomplete action="hospital_common.customreview.sfyonglchild.shengfangzl_xm" cols="1" label="项目名称" name="xmbh" data-fd="${fdname}" limit="10" value="${xmbh}" text="${xmmch}" >
					<s:option value="$[xmbh]" label="$[xmmch]"></s:option>
				</s:autocomplete>
				<s:textfield label="公式"  disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengfyl_child_02,shengfangzl_yongl_child'></s:textfield>
...
$(function(){
	...
	$("[name='xmbh']").change(function() {
		...
		$("[name='xmmch']").val('');
		if(value){
			$.call('hospital_common.customreview.sfyonglchild.shengfangzl_xm',{xmbh:sdata.xmbh},function(rtn){
				if(rtn.resultset){
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.customreview.sfyonglyf.save",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
