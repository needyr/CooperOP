...
<s:page title="编辑检验问题" ismodal="true">
	<s:row>
		<s:form id="form" label="检验问题">
			<s:toolbar>
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" disabled="disabled" value="${nianl_c}" dbl_action='zl_select_shengflabtest_06,shengfangzl_labtest'></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename"  cols="2" disabled="disabled" value="${routename}" dbl_action='zl_select_shengflabtest_07,shengfangzl_labtest' ></s:textfield>
			</s:row>
			<s:row>
				<input type="hidden" name="xmbh" value="${xmbh}" >
				<s:textfield label="项目名称" name="xmmch"  cols="1" disabled="disabled" value="${xmmch}" dbl_action='zl_select_shengflabtest_01,shengfangzl_labtest'></s:textfield>
				<%-- <s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield> --%>
				<s:textfield label="公式"  disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengflabtest_02,shengfangzl_labtest'></s:textfield>
...
<script type="text/javascript">
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.customreview.sflabtest.save",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "修改检验问题"}, function(){});
			}else{
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "新增检验问题"}, function(){});
			}
	     }
	}
