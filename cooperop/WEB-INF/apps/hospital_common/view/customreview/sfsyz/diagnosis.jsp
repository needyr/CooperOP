...
<s:page title="编辑适应症问题" ismodal="true">
	<s:row>
		<s:form id="form" label="适应症问题">
			...
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" disabled="disabled" value="${xmbh}" dbl_action='zl_select_shengfsyz_01,shengfangzl_syz'></s:textfield>
				<s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<s:textfield label="公式"  disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_shengfsyz_02,shengfangzl_syz'></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}" dbl_action='zl_select_shengfsyz_05,shengfangzl_syz'></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			sdata.id='${param.id}';
			$.call("hospital_common.customreview.sfsyz.save_mx",sdata,function(s){
				$.closeModal(s);
	    	});
			if(sdata.id){
