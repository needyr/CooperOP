...
<s:page title="选择角色" disloggedin="false">
<s:row>
<div class="top">
	<input type="text" name="search" placeholder="&nbsp;&nbsp;&nbsp;角色名称" class="search"/>
...
if('${param.code}'){
	var code = '${param.code}';
	$.call("hospital_common.ykrole.searchCheck",{code: code},function(rtn){
		if(rtn.role){
			var html = [];
			var v = rtn.role;
			for(i=0;i<v.length;i++){
				html.push('<tr class="is_c'+v[i].id+'" data_id="'+v[i].id+'">');
				html.push('<td width=100px>'+v[i].id+'</td>');
...
function search1(){
	...
	/* if(data){ */
		$.call("hospital_common.ykrole.search",{data: data,type:type},function(rtn){
			if(rtn.role){
				var html = [];
				var v = rtn.role;
				for(i=0;i<v.length;i++){
