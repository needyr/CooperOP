...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="cs" init-action="" flag="cs" description="cs" system_product_code="hospital_common" title="" remark="" othermxsql="" type="document"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("hospital_common.document.cs.cs", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("hospital_common.document.cs.cs", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "hospital_common.document.cs.cs", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("hospital_common.document.cs.cs", null);
	}
...
<s:row><s:form label="表单" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4"><s:row type="frow"><s:textfield label="textfield" cols="">null</s:textfield>
</s:row>
</s:form>
</s:row>
</s:page>
