...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="JCX012" init-action="" description="HIS入院病情字典" system_product_code="hospital_common" title="" remark="" flag="q" type="query"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("hospital_common.query.q.JCX012", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("hospital_common.query.q.JCX012", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "hospital_common.query.q.JCX012", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("hospital_common.query.q.JCX012", null);
	}
...
<s:row><s:form label="HIS入院病情字典" icon="icon-book-open" color="red" border="1" cols="4" istoorbar="N" app_collapsed="N"><s:row type="frow"><s:button label="查询" cols="" nextfocusfield="" icon="glyphicon glyphicon-play-circle" action="querymx" color=""></s:button>
</s:row>
</s:form>
<s:table label="HIS入院病情字典" cols="4" color="grey-silver" isdesign_="Y" tableid="JCX012" icon="" djselect="" tablekey="" sort="false" autoload="false" fields="" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="adm_condition_code" fdtype="字符" format="" dictionary="" label="入院病情代码" size="128" maxlength="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="" app_field=""></s:table.field>
<s:table.field name="adm_condition_name" fdtype="字符" format="" dictionary="" label="入院病情名称" size="128" maxlength="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:row>
</s:page>
