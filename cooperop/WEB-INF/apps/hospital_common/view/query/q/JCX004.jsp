...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="JCX004" init-action="" description="HIS疾病字典" system_product_code="hospital_common" title="" remark="" flag="q" type="query"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("hospital_common.query.q.JCX004", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("hospital_common.query.q.JCX004", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "hospital_common.query.q.JCX004", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("hospital_common.query.q.JCX004", null);
	}
...
<s:row><s:form label="HIS疾病字典" icon="icon-book-open" color="red" border="1" cols="4" istoorbar="N" app_collapsed="N"><s:row type="frow"><s:button label="查询" cols="" nextfocusfield="" icon="glyphicon glyphicon-play-circle" action="querymx" color=""></s:button>
</s:row>
</s:form>
<s:table label="HIS疾病字典" cols="4" color="grey-silver" isdesign_="Y" tableid="JCX004" icon="" djselect="" tablekey="" sort="false" autoload="false" fields="" limit="25" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="diagnosis_code" fdtype="字符" format="" dictionary="" label="诊断编号" size="" maxlength="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="" app_field=""></s:table.field>
<s:table.field name="diagnosis_name" fdtype="字符" format="" dictionary="" label="疾病名称" size="128" maxlength="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:row>
</s:page>
