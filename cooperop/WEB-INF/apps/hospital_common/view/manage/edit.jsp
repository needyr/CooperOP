...
<s:page title="表结构信息" ismodal="true">
	<s:row>
		<s:form label="表结构信息" id="fform">
			<s:toolbar>
				<s:button icon="glyphicon glyphicon-floppy-saved" size="btn-sm btn-default" label="保存" onclick="save()" type="submit"></s:button>
				<s:button icon="fa fa-ban" size="btn-sm btn-default" label="取消" onclick="$.closeModal();"></s:button>
			</s:toolbar>
			<s:row>
				<input type="hidden" name="id" value="${id }"></input>
				<s:textfield cols="4" label="表名称" name="name" value="${pageParam.tablename}"></s:textfield>
			</s:row>
		</s:form>
	</s:row>
		...
		<s:table label="表结构信息" limit="0" sort="true" id="subtable" isdesign="true">
			...
			<s:table.fields>
				...
				<s:table.field name="datatype" label="数据类型" datatype="template">
...
<script type="text/javascript">
	...
	function save() {
		...
		var page = "hospital_common.manage.insert";
		if ($("input[name='id']").val()) {
			page = "hospital_common.manage.update";
		} else {
		...
		$("#subtable").find("tbody").find("tr").each(function () {
			...
			if (_tdata.colname && _tdata.colname != "data_service_quene_id" && 
