...
<div class="left">
	<div class="thead_Div" id="main_thead">
		<table class="mytable">
			<thead>
				<tr>
					<th width=60px>点评编号</th>
					<th width=140px>点评名称</th>
				</tr>
			</thead>
		</table>
	</div>
...
if('${param.code}'){
	var code = '${param.code}';
	$.call("hospital_common.dictsyscomment.searchCheck",{code: code.split(',')},function(rtn){
		if(rtn.syscomment){
			var html = [];
			var v = rtn.syscomment;
			for(i=0;i<v.length;i++){
			html.push('<tr class="is_c'+v[i].system_code+'" data_id="'+v[i].system_code+'">');
			html.push('<td width=60px>'+v[i].system_code+'</td>');
			html.push('<td width=140px>'+v[i].comment_name+'</td>');
			html.push('</tr>');
			}
...
function search1(){
	...
	$.call("hospital_common.dictsyscomment.search",{data: data},function(rtn){
		if(rtn.syscomment){
			var html = [];
			var v = rtn.syscomment;
			for(i=0;i<v.length;i++){
			...
			$('#start_choose tbody tr').each(function(){
				var data ='[data_id='+'"'+$(this).attr("data_id")+'"'+']';
				$(data).attr('class','');
			});
