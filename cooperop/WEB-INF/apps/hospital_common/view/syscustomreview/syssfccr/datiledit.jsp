...
<s:page title="肌酐清除率" ismodal="true">
	<s:row>
		<s:form id="form" label="肌酐清除率">
			<s:toolbar>
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_sys_shengfccr_06,sys_shengfangzl_Ccr' disabled="disabled"></s:textfield>
			...
			<s:row>
				<s:textfield label="肌酐清除率" name="ccr_start"  cols="1" value="${ccr_start}" required="true"></s:textfield>
				<s:textfield label="到" name="ccr_end"  cols="1" value="${ccr_end}" required="true"></s:textfield>
				ml/min
			</s:row>
			<s:row>
				<s:textfield label="使用频率(N次/日)" name="frequency_n"  cols="3" value="${frequency_n}" dbl_action='zl_select_sys_shengfccr_07,sys_shengfangzl_Ccr' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" value="${xmbh}" dbl_action='zl_select_sys_shengfccr_01,sys_shengfangzl_Ccr' disabled="disabled"></s:textfield>
				<s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<s:textfield label="公式" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_sys_shengfccr_02,sys_shengfangzl_Ccr' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}"  dbl_action='zl_select_sys_shengfccr_05,sys_shengfangzl_Ccr' ></s:textfield>
			</s:row>
			<s:row>
				<s:textarea label="警示消息" name="nwarn_message" cols="4" placeholder="请输入警示提示消息内容"  rows="" >${nwarn_message}</s:textarea>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.syscustomreview.syssfccr.save",sdata,function(s){
				$.closeModal(s);
	    	});
			/* if(sdata.id){
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "修改肌酐清除率"}, function(){});
			}else{
				$.call("hospital_common.dictdrug.updateCz", 
						{drug_code: '${param.spbh}', zdy_cz: "新增肌酐清除率"}, function(){});
			} */
	     /* } */
	}
