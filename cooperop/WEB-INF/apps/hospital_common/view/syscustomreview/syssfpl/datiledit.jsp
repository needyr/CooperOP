	...
	<s:row>
		<s:form id="form" label="频率问题">
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_sys_shengfpl_06,sys_shengfangzl_pl' disabled="disabled"></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename"  cols="2" value="${routename}" dbl_action='zl_select_sys_shengfpl_07,sys_shengfangzl_pl' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" value="${xmbh}" dbl_action='zl_select_sys_shengfpl_01,sys_shengfangzl_pl' disabled="disabled"></s:textfield>
				<s:textfield label="项目名称" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<s:textfield label="公式" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_sys_shengfpl_02,sys_shengfangzl_pl' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}"     dbl_action='zl_select_sys_shengfpl_05,sys_shengfangzl_pl' ></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		/* if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.syscustomreview.syssfpl.save",sdata,function(s){
				$.closeModal(s);
	    	});
			/* if(sdata.id){
