	...
	<s:row>
		<s:form id="form" label="适应症问题">
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" disabled="disabled" value="${nianl_c}" dbl_action='zl_select_sys_shengfsyz_06,sys_shengfangzl_syz'></s:textfield>
			...
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" disabled="disabled" value="${xmbh}" dbl_action='zl_select_sys_shengfsyz_01,sys_shengfangzl_syz'></s:textfield>
				<s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<input type="hidden" name="formul" value="类似" />
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}" dbl_action='zl_select_sys_shengfsyz_05,sys_shengfangzl_syz'></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			$.call("hospital_common.syscustomreview.syssfsyz.save",sdata,function(s){
				$.closeModal(s);
	    	});
			/* if(sdata.id){
