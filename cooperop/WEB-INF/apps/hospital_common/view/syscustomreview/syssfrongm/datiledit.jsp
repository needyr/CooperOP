	...
	<s:row>
		<s:form id="form" label="溶媒问题">
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" disabled="disabled" value="${nianl_c}" dbl_action='zl_select_sys_shengfrm_06,sys_shengfangzl_rongm'></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename"  cols="2" disabled="disabled" value="${routename}" dbl_action='zl_select_sys_shengfrm_07,sys_shengfangzl_rongm' ></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" disabled="disabled" value="${xmbh}" dbl_action='zl_select_sys_shengfrm_02,sys_shengfangzl_rongm'></s:textfield>
				<s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<s:textfield label="公式"  disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_sys_shengfrm_03,sys_shengfangzl_rongm'></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}"     dbl_action='zl_select_sys_shengfrm_01,sys_shengfangzl_rongm'></s:textfield>
			</s:row>
			<s:row>
	...
	function save(){
		...
		if(str2=='' && str3=='' || rex.test(str2) && rex.test(str3)){
			if (patrn.test(str)) {// 如果包含特殊字符
				...
				$.call("hospital_common.syscustomreview.syssfrongm.save",sdata,function(s){
					$.closeModal(s);
		    	});
				/* if(sdata.id){
