...
<s:page title="编辑相互作用问题" ismodal="true">
	<s:row>
		<s:form id="form" label="相互作用问题">
			...
			<s:row>
				<s:textfield label="年龄段" name="nianl_c"  cols="1" value="${nianl_c}" dbl_action='zl_select_sys_shengfxhzy6,sys_shengfangzl_xhzy' disabled="disabled"></s:textfield>
			...
			<s:row>
			    <s:textfield label="给药方式like" name="routename" value="${routename}"  cols="2"  dbl_action='zl_select_sys_shengfxhzy7,sys_shengfangzl_xhzy' disabled="disabled"></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="项目编号" name="xmbh"  cols="1" value="${xmbh}" dbl_action='zl_select_sys_shengfxhzy2,sys_shengfangzl_xhzy' disabled="disabled"></s:textfield>
				<s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}"></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<s:textfield label="公式" disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_sys_shengfxhzy3,sys_shengfangzl_xhzy'></s:textfield>
			</s:row>
			<s:row>
				<s:textfield label="值" name="value"  cols="2" value="${value}"    dbl_action='zl_select_sys_shengfxhzy1,sys_shengfangzl_xhzy' ></s:textfield>
			</s:row>
			<s:row>
...
<script type="text/javascript">
	...
	function save(){
		...
		if (patrn.test(str)) {// 如果包含特殊字符
			...
			sdata.id='${param.id}';
			$.call("hospital_common.syscustomreview.syssfxhzy.save",sdata,function(s){
				$.closeModal(s);
	    	});
			/* if(sdata.id){
