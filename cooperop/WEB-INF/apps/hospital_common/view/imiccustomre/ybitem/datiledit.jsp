...
<s:page title="医保项目规则" ismodal="true">
	<s:row>
		<s:form id="form" label="医保项目规则">
			<s:toolbar>
			...
			<s:row>
				...
				<s:textfield label="年龄段" name="nianl_c"  cols="1" disabled="disabled" value="${nianl_c}" dbl_action='zl_select_yb_item_01,yb_shengfangzl_item'></s:textfield>
			...
			<s:row>
				...
				<s:textfield label="项目名称" disabled="disabled" name="xmmch"  cols="1" value="${xmmch}" dbl_action='zl_select_yb_item_02,yb_shengfangzl_item'></s:textfield>
				<input  hidden="true"  name="xmid"  value="${xmid}">
				<s:textfield label="公式"  disabled="disabled" name="formul"  cols="1" value="${formul_name}" dbl_action='zl_select_yb_item_03,yb_shengfangzl_item'></s:textfield>
	...
	function save(){
		...
		//if (patrn.test(str)) {// 如果包含特殊字符
	     ...
	     //}else{
			...
			$.call("hospital_common.imiccustomre.ybitem.save",sdata,function(s){
				$.closeModal(s);
	    	});
	     //}
	}
