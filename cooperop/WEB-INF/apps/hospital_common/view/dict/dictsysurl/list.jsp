...
<s:page title="系统链接库维护">
	<s:row>
		<s:form id="form" label="快速查找">
			<s:row>
				<s:textfield label="关键字" name="filter" placeholder="请输入链接地址或名称" cols="2"></s:textfield>
				<s:button label="查询" icon="fa fa-search" onclick="query()"></s:button>
			</s:row>
		</s:form>
	</s:row>
	<s:row>
		<s:table sort="true" id="datatable" label="系统链接列表" autoload="false" action="hospital_common.dict.dictsysurl.query"  >
			<s:toolbar>
				<s:button icon="fa fa-plus" label="新增" onclick="Add()"></s:button>
			</s:toolbar>
			<s:table.fields>
				<s:table.field name="id" label="编号"  sort="true"></s:table.field>
				<s:table.field name="url_name" label="链接名称"></s:table.field>
...
function del(id){
	$.call("hospital_common.dict.dictsysurl.delete", {id: id}, function(rtn){
		if(rtn){
			query();
		}
	})
