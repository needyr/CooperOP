...
public class WebtabstructDao extends BaseDao{
	...
	public int insert(Map<String,Object> params) throws Exception{
		...
		sql.append("  data_webservice_quene_id  numeric(18, 0) not null,");
		sql.append("  id  numeric(18, 0)  not null,");
		sql.append("  parent_id  numeric(18, 0) default 0  not null  Primary Key (data_webservice_quene_id, id, parent_id),");
		//表结构list
...
//			sql.append("  " + map.get("colname") + " ");
	...
	public int dropTable(Map<String,Object> params) throws Exception{
		...
		// +++
		Record r_tablename = new Record();
		r_tablename.put("tablename", name);
		List<Record> fields = query(r_tablename).getResultset();
		for(Map<String,Object> map : fields){
			String colname = (String) map.get("name");
			Record r_alias = new Record();
			r_alias.put("table_name", name);
			r_alias.put("field_name", colname);
	...
	public int modify(Map<String,Object> params) throws Exception {
		...
		for (Map<String, Object> mfIn : topFields.getResultset()) {
			if ("data_webservice_quene_id".equals(mfIn.get("name")) || 
