...
<s:page title="说明书" disloggedin="true">
<style>
.portlet {
...
<head>
 <meta charset="utf-8">
 <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/res/ipc/css/instruction.css">
 <body>
  <div class="main" >
   <div class="quick">
    	<s:form id ="dataForm">
    		<s:row>
    			<s:autocomplete id="" name="drug_code" action="hospital_common.additional.queryDrug"
					value="${drug_code}" text="${drug_name}" limit="10" cols="1" editable="true" placeholder="请输入药品编号，名称，拼音码">
				...
				<%-- <input name="cg_query" autocomplete="off" style="width: calc(30% - 120px);">
    ...
    <nav class="leftList" id="myScrollspy" >
     <ul class="">
      <c:forEach items="${$return.smsdrug}" var="sms">
   		 <c:forEach items="${$return.drug}" var="dr">
...
<script type="text/javascript">
 var drugcode='${drug.sys_drug_code}'
 var his_drugcode='${drug.drug_code}';
if(!drugcode){
	$.message("暂未收录该药品说明书，敬请期待！");
} 
$(function(){
	aByClick();
	...
	$('#ckpdf').click(function(){
		$.modal('/w/hospital_common/additional/shuoms_img.html?drug_code='+his_drugcode,"药品说明书附件",{
	        width:"95%",
	        height:"95%",
	        maxmin: true,
...
function getInstruction(){
	...
	$.call("hospital_common.additional.instruction", data, function(rtn){
		...
		if(rtn != null && rtn.drug != null && rtn.drug != 0){
			his_drugcode = rtn.drug.drug_code;
			$('#ckpdf').remove();
...
function ckpdf(){
	/* layer.open({
		...
		shade: 0,
		area: ['95%', '95%'],//定义宽和高
		content: '/w/hospital_common/additional/shuoms_img.html?drug_code='+'${drug.drug_code}',//打开的内容
		success: function(layero, index){
		}
	}) */
	$.modal('/w/hospital_common/additional/shuoms_img.html?drug_code='+'${drug.drug_code}',"药品说明书附件",{
        width:"95%",
        height:"95%",
        maxmin: true,
		shadeClose: false,//点击遮罩不关闭层
