...
<s:page title="说明书管理" >
	...
	<s:row>
		<s:table id="datatable" label="说明书管理" autoload="false" action="ipc.specification.query" sort="true" >
			<s:toolbar>
				<s:button icon="" label="新增说明书" onclick="Add()"></s:button>
			</s:toolbar>
			<s:table.fields>
				<s:table.field name="sys_drug_code" label="药品编号" datatype="" ></s:table.field>
				<s:table.field name="name_cn" label="药品名称" datatype="script" >
					if(record.name_en){
						return record.name_cn+"["+record.name_en+"]"
					}
					return record.name_cn
				</s:table.field>
				<s:table.field name="paking" label="包装规格" ></s:table.field>
				<s:table.field name="approval_number" label="批准文号" width="250"></s:table.field>
				<s:table.field name="manufacturer_short_name" label="生产厂家简称" datatype="" ></s:table.field>
...
<script type="text/javascript">
	...
	function Delete(sys_drug_code){
		$.confirm("是否确认删除？删除后无法恢复！",function callback(e){
			if(e==true){
				$.call("ipc.specification.delete",{"sys_drug_code":sys_drug_code},function (rtn){
					query();
				})
			}
		})	
