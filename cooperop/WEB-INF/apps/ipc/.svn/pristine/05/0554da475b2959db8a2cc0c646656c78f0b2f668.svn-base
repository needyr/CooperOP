...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/res/ipc/css/precheck.css" >
...
<style type="text/css">
#commentright{width: 100%;height: 100%;}
.list p{margin : 0 0 0 0 ;}
.list label{cursor: pointer;}
::-webkit-scrollbar {width: 2px;height: 4px;background-color: #F5F5F5;}
.hasreport{display: inline-block !important;width: 450px;height: 60px;text-align: center;padding: 3px;color: #bb7d0f;font-size: 16px;}
</style>
...
<body>
    <div class="wk">
	    <div class="botom">
		    <div class="btn-content" style="z-index:99999">
			     <a class="caozuo2 qd-button" id="qd2" style="margin:20px 20px 0px 0px !important" onclick="cancel();">取消用药</a>
			     <a class="sqmyy" id="sqmyy" style="margin:20px 20px 0px 0px !important" onclick="sqmyy();" >用药问题再次确认</a>
		    </div>
	    </div>
        <!-- 左边导航栏 -->
        <div class="left-content">
            <ul>
			    ...
			    <li class="nav-content" title="${tmum.sort_name}">
			        <a href="javascript:;" data-item-id="${tmum.sort_id }">${fn:substring(tmum.sort_name, 0, 8)}${fn:length(tmum.sort_name) > 9 ?  '...': ''}(<b>${tmum.num }</b>)</a>
			    </li>
			    </c:forEach>
			    </c:if>
				...
				<li class="nav-content" title="${item.sort_name }">
				    <a href="javascript:;" data-item-id="${item.sort }"> ${item.sort_name } (<b>${item.num }</b>)</a>
				</li>
				</c:forEach>
				</c:if>
            </ul>
...
var source_type = '';
	$(function(){
		...
		if('${param.msg_alert_id}'){
			msg_alert_id = '${param.msg_alert_id}';
		}else if('${$return.msg_alert_id}'){
			msg_alert_id = '${$return.msg_alert_id}';
		}
		if('${param.userinfo_id}'){
			userinfo_id = '${param.userinfo_id}';
		}else if ('${$return.userinfo_id}'){
			userinfo_id = '${$return.userinfo_id}';
		}
	...
	function cancel(){
		var tj = '请 <font style="color: red;font-size: 16px;">修改</font> 患者<font style="color: #e2b011;font-size: 16px;">【' +'${$return.patient.patient_name}' + '】</font>的医嘱/处方';
		var data ={
			"auto_audit_id": '${$return.ipc_auto_id}', 
			"doctor_no": '${$return.auto.doctor_no}', 
			state: "DN",
			flow_id: '${$return.flow_id}',
			task_id: '${param.task_id}',
			audited: 'Y',
			djbh: '${$return.djbh}',
			'msg_alert_id': msg_alert_id,
			uid: '${param.uid}'
		};
		/* //删除弹窗提示,后台将is_sure改为1
		$.call("ipc.autoaudit.update",{id: '${$return.ipc_auto_id}',is_sure: 1},function(rtn){
			if(typeof crtech != "undefined"){crtech.modal("/w/ipc/auditresult/notice.html", "250", "85");}
		}); */
		
        $.call("ipc.auditflow.ChangeState", data, function(rtn){
		    if(rtn){
		    	if(rtn.is_success=='success'){
		    		layer.open({
					    title: '系统提示',
					    btn: ['知道了'],
					    content: tj,
						closeBtn: 0,
						yes:function(){
						    /* try {$.closeModal(true);}catch(err) {
						    	//location.href='/w/application/task/mine.html'+"?_CRSID="+'${_CRSID}';
						    	window.close();
					    	} */
	...
	function sqmyy(){
		//var tj = '请 <font style="color: #e2b011;font-size: 16px;">发送</font> 患者<font style="color: #e2b011;font-size: 16px;">【' +'${$return.patient.patient_name}' + '】</font>的医嘱/处方';
		var tj = '已发送患者<font style="color: #e2b011;font-size: 16px;">【' +'${$return.patient.patient_name}' + '】</font>的医嘱/处方 至 护士站';
		var data ={
				"auto_audit_id": '${$return.ipc_auto_id}', 
				"doctor_no": '${$return.auto.doctor_no}', 
				state: "DS",
		...
		$.call("ipc.auditflow.ChangeState", data, function(rtn){
			if(rtn){
				if ('${$return.orders[0].p_type}' == '2'){
					tj = '<font style="color: #F85B5E;font-size: 16px;">【请在纸质处方上写明原因，并双签名。】</font>';
				}
				layer.open({
					 title: '系统提示',
					 btn: ['知道了'],
