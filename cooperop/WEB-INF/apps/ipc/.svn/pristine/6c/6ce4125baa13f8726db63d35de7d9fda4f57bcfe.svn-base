...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/res/ipc/css/comment.css" >
 	<div class="col-md-9">
			...
			<div id="btn_ss" class="fa fa-angle-double-down"></div>
		<s:row>
		<s:tabpanel >
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#datatable").find("tbody").on("click", "tr", function() {
			...
			var wthtml = [];
			$.call("ipc.comment.getCommenting",gxdata,function(rtn){
				wthtml.push('<ur>');
				if(rtn){
					for(i=0;i<rtn.checker.length;i++){
						wthtml.push('<li title="'+rtn.checker[i].comment_name+'">');
						wthtml.push(rtn.checker[i].comment_name);
						wthtml.push('</li>');
					} 
					wthtml.push('</ur>');
					$("#comment_reason").text(rtn.writer.comment_content == null ? "未填写点评理由":rtn.writer.comment_content);
					var comment_result_ne = rtn.writer.comment_result;
