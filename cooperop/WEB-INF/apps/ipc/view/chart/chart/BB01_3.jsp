...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB01_3" init-action="" description="审查严重程度[更多]->科室" system_product_code="ipc" title="审查严重程度[更多]➩科室" remark="审查严重程度[更多]➩科室" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB01_3", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB01_3", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB01_3", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB01_3", null);
	}
...
<s:row><s:form label="筛选条件" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="Y"><s:row type="frow"><s:textfield label="严重程度" cols="1" name="check_level_name" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="50" precision="">${empty check_level_name ? '' : check_level_name}</s:textfield>
...
<s:row><s:tabpanel color="grey-silver" cols="4"><s:table label="审查严重程度[更多]➩科室" cols="4" color="grey-silver" active="true" isdesign_="Y" tableid="01" icon="glyphicon glyphicon-sort-by-attributes" autoload="true" toIndex="0" sort="true" myIndex="0" djselect="ipc.chart.chart.BB01_3_1" subpage_name="审查严重程度[更多]➩科室[:dept_name]➩医生" limit="25" fitwidth="true" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="dept_name" value="$[dept_name]"  fdtype="字符" format="" dictionary="" label="科室名称" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="150" fieldorder="0" is_href="ipc.chart.chart.BB01_3_1" app_field="" datatype="template"><s:textfield readonly="true" name="dept_name" value="$[dept_name]"  fdtype="字符" format="" dictionary="" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="150" fieldorder="0" is_href="ipc.chart.chart.BB01_3_1" app_field="">null</s:textfield>
</s:table.field>
<s:table.field name="禁忌" value="${empty 禁忌 ? '' : 禁忌}" fdtype="实数	" format="" dictionary="" label="禁忌" size="" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="60" fieldorder="1" is_href="" app_field=""></s:table.field>
<s:table.field name="不推荐" value="${empty 不推荐 ? '' : 不推荐}" fdtype="实数	" format="" dictionary="" label="不推荐" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="60" fieldorder="2" is_href="" app_field=""></s:table.field>
<s:table.field name="慎用" value="${empty 慎用 ? '' : 慎用}" fdtype="实数	" format="" dictionary="" label="慎用" size="50" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="60" fieldorder="3" is_href="" app_field=""></s:table.field>
<s:table.field name="关注" value="${empty 关注 ? '' : 关注}" fdtype="实数	" format="" dictionary="" label="关注" size="50" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="60" fieldorder="4" is_href="" app_field=""></s:table.field>
<s:table.field name="dept_code" value="${empty dept_code ? '' : dept_code}" fdtype="字符" format="" dictionary="" label="科室代码" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="60" fieldorder="5" is_href="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:tabpanel>
</s:row>
