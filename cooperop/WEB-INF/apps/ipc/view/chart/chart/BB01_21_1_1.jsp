...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB01_21_1_1" init-action="" description="审查不合格分类[下钻]->科室->医生->病人" system_product_code="ipc" title="审查不合格分类[下钻]➩科室➩医生➩病人" remark="审查不合格分类[更多]_科室_医生_病人" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB01_21_1_1", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB01_21_1_1", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB01_21_1_1", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB01_21_1_1", null);
	}
...
<s:row><s:form label="审查结果下钻" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="Y"><s:row type="frow"><s:textfield label="审查类型" cols="1" name="xaxis" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="50" precision="">${empty xaxis ? '' : xaxis}</s:textfield>
...
<s:radio color="grey" label="" name="is_after" value="${empty is_after ? '' : is_after}" placeholder="是否事后审查" fdtype="字符" cols="1" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" clabel="全部" contentIndex="0" contentvalue="0" maxlength="2" precision=""><s:option label="全部" contentIndex="0" value="0"></s:option>
...
<s:row><s:tabpanel color="grey-silver" cols="4"><s:table label="审查不合格分类[下钻]➩科室➩医生➩患者" cols="4" color="grey-silver" active="true" isdesign_="Y" tableid="01" icon="icon-user-following" autoload="true" toIndex="0" sort="false" myIndex="0" djselect="" subpage_name="" limit="25" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="dept_name" value="${empty dept_name ? '' : dept_name}" fdtype="字符" format="" dictionary="" label="科室名称" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="100" fieldorder="0" is_href="" app_field=""></s:table.field>
<s:table.field name="patient_name" value="$[patient_name]"  fdtype="字符" format="" dictionary="" label="患者姓名" size="1280" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="100" fieldorder="1" is_href="hospital_common.patient.index" app_field="" datatype="template"><s:textfield readonly="true" name="patient_name" value="$[patient_name]"  fdtype="字符" format="" dictionary="" size="1280" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="100" fieldorder="1" is_href="hospital_common.patient.index" app_field="">null</s:textfield>
</s:table.field>
<s:table.field name="doctor" value="${empty doctor ? '' : doctor}" fdtype="字符" format="" dictionary="" label="医生" size="32" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="2" is_href="" app_field=""></s:table.field>
...
<s:table.field name="ordermessage" value="${empty ordermessage ? '' : ordermessage}" datatype="script"  fdtype="字符" format="" dictionary="" label="医嘱信息" size="2000" maxlength_show="60" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="5" is_href="" app_field=""> var html = [];  if(record.ordermessage && record.ordermessage.length>60){  	html.push('<a href="javascript:void(0);" title="'+record.ordermessage+'" style="cursor: pointer;"');  	html.push(' onclick="show_maxlength(this);" ');  	html.push('>');  	html.push(record.ordermessage.substring(0,60)+'……');  	html.push('</a>');  }else{  	html.push(record.ordermessage);  }  return html.join(''); </s:table.field>
...
<s:table.field name="check_datetime" value="${empty check_datetime ? '' : check_datetime}" fdtype="字符" format="" dictionary="" label="审查日期" size="" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="150" fieldorder="9" is_href="" app_field=""></s:table.field>
