...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB04_11" init-action="" description="点评不合格比例[下钻]->药品分类数量" system_product_code="ipc" title="点评不合格比例[下钻]->药品分类数量" remark="点评不合格比例[下钻]->药品分类数量" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB04_11", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB04_11", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB04_11", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB04_11", null);
	}
...
<s:row><s:table label="点评不合格比例[下钻]->药品分类数量" cols="4" color="grey-silver" isdesign_="Y" tableid="01" icon="" djselect="ipc.chart.chart.BB04_11_1" subpage_name="点评不合格比例[下钻]->药品分类数量->患者信息" autoload="true" sort="true" limit="25" fitwidth="false" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="property_toxi" value="${empty property_toxi ? '' : property_toxi}" fdtype="字符" format="" dictionary="" label="药品分类" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="250" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="shl" value="${empty shl ? '' : shl}" fdtype="实数" format="" dictionary="" label="抽样数量" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="150" fieldorder="" is_href="" app_field=""></s:table.field>
