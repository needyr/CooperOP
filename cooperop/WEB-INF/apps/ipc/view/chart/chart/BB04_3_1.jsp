...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB04_3_1" init-action="" description="点评问题分类[更多]->问题结果->患者信息" system_product_code="ipc" title="点评问题分类[更多]->问题结果->患者信息" remark="点评问题分类[更多]->问题结果->患者信息" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB04_3_1", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB04_3_1", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB04_3_1", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB04_3_1", null);
	}
...
<s:row><s:form label="筛选条件" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="Y"><s:row type="frow"><s:textfield label="点评问题名称" cols="2" name="comment_name" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="128" precision="">${empty comment_name ? '' : comment_name}</s:textfield>
</s:row>
...
<s:row><s:table label="点评问题分类[更多]->问题结果->患者信息" cols="4" color="grey-silver" isdesign_="Y" tableid="02" icon="" djselect="" subpage_name="" autoload="true" sort="false" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="patient_name" value="$[patient_name]"  fdtype="字符" format="" dictionary="" label="患者姓名" size="120" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="150" fieldorder="" is_href="hospital_common.showturns.patientdetail" app_field="" datatype="template"><s:textfield readonly="true" name="patient_name" value="$[patient_name]"  fdtype="字符" format="" dictionary="" size="120" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="150" fieldorder="" is_href="hospital_common.showturns.patientdetail" app_field="">null</s:textfield>
</s:table.field>
