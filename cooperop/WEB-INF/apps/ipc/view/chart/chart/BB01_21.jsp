...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB01_21" init-action="" description="审查不合格分类[下钻]->科室" system_product_code="ipc" title="审查不合格分类[下钻]➩科室" remark="审查不合格分类[下钻]➩科室" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB01_21", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB01_21", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB01_21", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB01_21", null);
	}
...
<s:row><s:tabpanel color="grey-silver" cols="4"><s:table label="审查不合格分类[下钻]➩科室" cols="4" color="grey-silver" active="true" isdesign_="Y" tableid="01" icon="glyphicon glyphicon-sort-by-attributes" autoload="true" toIndex="0" sort="true" myIndex="0" djselect="ipc.chart.chart.BB01_21_1" subpage_name="审查不合格分类[下钻]➩科室[{:dept_name}]➩医生" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="dept_name" value="${empty dept_name ? '' : dept_name}" fdtype="字符" format="" dictionary="" label="科室名称" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="center" width="150" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="shl" value="${empty shl ? '' : shl}" fdtype="实数" format="" dictionary="" label="问题数量" size="14" maxlength_show="" digitsize="3" precision="" available="true" sort="true" controltype="" align="center" width="160" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="count_shl" value="${empty count_shl ? '' : count_shl}" fdtype="实数	" format="" dictionary="" label="问题总数量" size="50" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="center" width="160" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="shl_bl" value="${empty shl_bl ? '' : shl_bl}" fdtype="实数	" format="" dictionary="" label="问题占比" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="center" width="160" fieldorder="" is_href="" app_field=""></s:table.field>
