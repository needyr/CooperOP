...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB04_2_1_1_1" init-action="" description="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名->患者信息" system_product_code="ipc" title="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名->患者信息" remark="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名->患者信息" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB04_2_1_1_1", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB04_2_1_1_1", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB04_2_1_1_1", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB04_2_1_1_1", null);
	}
...
<s:row><s:form label="筛选条件" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="Y"><s:row type="frow"><s:textfield label="医生" cols="1" name="doctor" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="32" precision="">${empty doctor ? '' : doctor}</s:textfield>
</s:row>
