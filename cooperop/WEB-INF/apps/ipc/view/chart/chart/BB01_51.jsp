...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB01_51" init-action="" description="审查不合格科室[下钻]->审查类型" system_product_code="ipc" title="审查不合格科室[下钻]➩审查类型" remark="审查不合格科室[下钻]➩审查类型" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB01_51", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB01_51", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB01_51", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB01_51", null);
	}
...
<s:row><s:tabpanel color="grey-silver" cols="4"><s:table label="审查不合格科室[下钻]➩审查类型" cols="4" color="grey-silver" active="true" isdesign_="Y" tableid="01" icon="glyphicon glyphicon-sort-by-attributes" autoload="true" toIndex="0" sort="true" myIndex="0" djselect="ipc.chart.chart.BB01_51_1" subpage_name="审查不合格科室[更多]➩[:sort_name]➩医生" limit="25" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="dept_name" value="${empty dept_name ? '' : dept_name}" fdtype="字符" format="" dictionary="" label="科室名称" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="0" is_href="" app_field=""></s:table.field>
<s:table.field name="sort_name" value="$[sort_name]"  fdtype="字符" format="" dictionary="" label="类别名称" size="50" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="200" fieldorder="1" is_href="ipc.chart.chart.BB01_51_1" app_field="" datatype="template"><s:textfield readonly="true" name="sort_name" value="$[sort_name]"  fdtype="字符" format="" dictionary="" size="50" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="textfield" align="left" width="200" fieldorder="1" is_href="ipc.chart.chart.BB01_51_1" app_field="">null</s:textfield>
</s:table.field>
<s:table.field name="关注" value="${empty 关注 ? '' : 关注}" fdtype="实数 " format="" dictionary="" label="关注" size="14" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="2" is_href="" app_field=""></s:table.field>
<s:table.field name="慎用" value="${empty 慎用 ? '' : 慎用}" fdtype="实数	" format="" dictionary="" label="慎用" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="3" is_href="" app_field=""></s:table.field>
<s:table.field name="不推荐" value="${empty 不推荐 ? '' : 不推荐}" fdtype="实数	" format="" dictionary="" label="不推荐" size="" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="4" is_href="" app_field=""></s:table.field>
<s:table.field name="禁忌" value="${empty 禁忌 ? '' : 禁忌}" fdtype="实数	" format="" dictionary="" label="禁忌" size="" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="5" is_href="" app_field=""></s:table.field>
<s:table.field name="sort_id" value="${empty sort_id ? '' : sort_id}" datatype="script"  fdtype="字符" format="" dictionary="" label="类别编号" size="60" maxlength_show="60" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="6" is_href="" app_field=""> var html = [];  if(record.sort_id && record.sort_id.length>60){  	html.push('<a href="javascript:void(0);" title="'+record.sort_id+'" style="cursor: pointer;"');  	html.push(' onclick="show_maxlength(this);" ');  	html.push('>');  	html.push(record.sort_id.substring(0,60)+'……');  	html.push('</a>');  }else{  	html.push(record.sort_id);  }  return html.join(''); </s:table.field>
<s:table.field name="dept_code" value="${empty dept_code ? '' : dept_code}" fdtype="字符" format="" dictionary="" label="科室代码" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="120" fieldorder="7" is_href="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:tabpanel>
</s:row>
