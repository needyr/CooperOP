...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB03_3_03" init-action="" description="审查时间统计[下钻]->科室->医生->审查记录" system_product_code="ipc" title="" remark="" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB03_3_03", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB03_3_03", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB03_3_03", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB03_3_03", null);
	}
...
<s:row><s:form label="选择条件" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="Y"><s:row type="frow"><s:textfield label="医生姓名" cols="1" name="doctorname" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="50" precision="">${empty doctorname ? '' : doctorname}</s:textfield>
...
<s:textfield label="医生编号" cols="1" name="doctor_no" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="20" precision="">${empty doctor_no ? '' : doctor_no}</s:textfield>
</s:row>
</s:form>
</s:row>
<s:row><s:table label="审查时间统计[下钻]->科室->医生->审查记录" cols="4" color="grey-silver" isdesign_="Y" tableid="04" icon="glyphicon glyphicon-sort-by-attributes" djselect="" subpage_name="" autoload="true" sort="false" limit="25" app_table="N" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="create_time" value="${empty create_time ? '' : create_time}" fdtype="字符" format="" dictionary="" label="提交审方时间" size="50" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="" is_href="" app_field=""></s:table.field>
...
<s:table.field name="pharmacist_exam_time" value="${empty pharmacist_exam_time ? '' : pharmacist_exam_time}" fdtype="字符" format="" dictionary="" label="药师审查时间" size="23" maxlength_show="" digitsize="" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="" is_href="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:row>
</s:page>
