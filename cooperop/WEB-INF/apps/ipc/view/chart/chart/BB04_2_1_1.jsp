...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="BB04_2_1_1" init-action="" description="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名" system_product_code="ipc" title="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名" remark="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.chart.chart.BB04_2_1_1", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.chart.chart.BB04_2_1_1", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.chart.chart.BB04_2_1_1", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.chart.chart.BB04_2_1_1", null);
	}
...
<s:row><s:table label="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名" cols="4" color="grey-silver" isdesign_="Y" tableid="03" icon="icon-user-following" djselect="ipc.chart.chart.BB04_2_1_1_1" subpage_name="点评不合格药品分类[更多]->药品分类->科室名称->医生姓名->患者信息" autoload="true" sort="false" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="doctor" value="${empty doctor ? '' : doctor}" fdtype="字符" format="" dictionary="" label="医生" size="32" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="250" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="shl" value="${empty shl ? '' : shl}" fdtype="字符" format="" dictionary="" label="数量" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="150" fieldorder="" is_href="" app_field=""></s:table.field>
