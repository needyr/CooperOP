...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="JCX066" init-action="" description="点评处方占处方总数的比例" system_product_code="ipc" title="点评处方占处方总数的比例" remark="点评处方占处方总数的比例" flag="q" type="query"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.query.q.JCX066", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.query.q.JCX066", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.query.q.JCX066", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.query.q.JCX066", null);
	}
...
<s:row type="frow"><s:textarea label="" cols="4" name="shuoming" placeholder="说明" fdtype="字符" defaultValue="需要选择时间段，按照科室进行统计 =点评处方数量/处方总数" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="300" precision="">${empty shuoming ? '需要选择时间段，按照科室进行统计 =点评处方数量/处方总数' : shuoming}</s:textarea>
</s:row>
</s:form>
</s:row>
<s:row><s:tabpanel color="grey-silver" cols="4"><s:table label="点评处方占处方总数的比例" cols="4" color="grey-silver" active="true" isdesign_="Y" tableid="01" icon="" djselect="" tablekey="" limit="25" toIndex="0" sort="true" autoload="false" istoorbar="N" myIndex="0" fields="" action="application.bill.queryTable"  select="multi" ><s:table.fields><s:table.field name="dept_name" fdtype="字符" format="" dictionary="" label="科室名称" size="128" maxlength_show="" digitsize="0" precision="" available="true" sort="true" controltype="" align="left" width="250" fieldorder="" is_href="" app_field=""></s:table.field>
<s:table.field name="dp_shl" fdtype="实数" format="" dictionary="" label="点评数量" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="" is_href="" app_field=""></s:table.field>
...
<s:table.field name="dpl" fdtype="实数" format="" dictionary="" label="点评率" size="14" maxlength_show="" digitsize="2" precision="" available="true" sort="true" controltype="" align="left" width="200" fieldorder="" is_href="" app_field=""></s:table.field>
</s:table.fields>
</s:table>
</s:tabpanel>
</s:row>
