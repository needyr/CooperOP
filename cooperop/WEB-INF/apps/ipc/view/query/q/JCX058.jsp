...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="JCX058" init-action="" description="科室患者审查详情" system_product_code="ipc" title="科室患者审查详情" remark="科室患者审查详情" flag="q" type="query"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.query.q.JCX058", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.query.q.JCX058", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.query.q.JCX058", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.query.q.JCX058", null);
	}
...
<s:row type="frow"><s:textfield color="grey" label="科室名称" name="dept_name" placeholder="" fdtype="字符" cols="1" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="zl_select_JCX058_02" out_action="" tableid="" expressions="" maxlength="128" precision="">${empty dept_name ? '' : dept_name}</s:textfield>
<s:radio label="是否在院" cols="1" name="in_hospital" value="${empty in_hospital ? '1' : in_hospital}" placeholder="" fdtype="字符" defaultValue="1" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" clabel="1在院" contentIndex="0" contentvalue="1" maxlength="10" precision=""><s:option label="1在院" contentIndex="0" value="1"></s:option>
<s:option label="0出院" contentIndex="1" value="0"></s:option>
</s:radio>
</s:row>
<s:row type="frow"><s:textfield label="患者ID/住院号" cols="1" name="patient_id" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="32" precision="">${empty patient_id ? '' : patient_id}</s:textfield>
