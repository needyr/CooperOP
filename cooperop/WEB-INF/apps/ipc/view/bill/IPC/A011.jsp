...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="A011" init-action="" flag="IPC" description="超说明书用药申请_科室主任审批" system_product_code="ipc" djzy="超说明书用药申请_科室主任审批" helpno="超说明书用药申请_科室主任审批" title="超说明书用药申请_科室主任审批" remark="" printmx="" sub="" modalwidth="" modalheight="" djprintsql="" othermxsql="" type="bill"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("ipc.bill.IPC.A011", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("ipc.bill.IPC.A011", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "ipc.bill.IPC.A011", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("ipc.bill.IPC.A011", null);
	}
...
<s:row><s:form label="超说明书用药申请_科室主任审批" icon="fa fa-list-alt" color="grey-silver" border="1" cols="4" istoorbar="N"><s:row type="frow"><s:textarea label="主任审核意见" cols="4" name="director_shenhe_message" placeholder="" fdtype="字符" defaultValue="同意" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="512" precision="">${empty director_shenhe_message ? '同意' : director_shenhe_message}</s:textarea>
</s:row>
<s:row type="frow"><s:button label="通  过" cols="" nextfocusfield="" icon="glyphicon glyphicon-ok" action="approval" color="blue"></s:button>
<s:button label="驳  回" cols="" nextfocusfield="" icon="glyphicon glyphicon-remove" action="reject" color=""></s:button>
</s:row>
...
<s:row type="frow"><s:textfield label="药品代码" cols="1" name="drug_code" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="true" required="true" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="32" precision="">${empty drug_code ? '' : drug_code}</s:textfield>
...
<s:image label="图片名称" cols="4" accept="image/*" name="imagename" value="${empty imagename ? '' : imagename}" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" deleteable="true" addable="true" downloadable="true" create_action="" modify_action="" enter_action="" dbl_action="" out_action="" tableid="" expressions="" maxlength="512" precision=""></s:image>
</s:row>
