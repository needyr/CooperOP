...
<s:row>
	...
	<s:form label="查询页面属性" icon="fa fa-gift" id="setFrom">
		...
		<s:row>
			<s:autocomplete label="再现单据" name="djselect" action="crdc.designer.query" value="${table.djselect}" editable="true">
		...
		<s:row>
			<s:textarea label="查询sql" name="initsql" cols="2">${table.initsql }</s:textarea>
			<s:textarea label="计数sql" name="countsql" cols="2">${table.countsql }</s:textarea>
...
<div class="row">
	<s:table label="字段设置" select="multi" id="table_" fitwidth="true" limit="50" isdesign="true">
		<s:toolbar>
			<s:button onclick="initfields();return false;" color="red" label="初始化明细字段"></s:button>
...
<script type="text/javascript">
	...
	function save() {
		...
		/* if(d.initsql && (d.initsql.indexOf(">") > -1 || d.initsql.indexOf("<") > -1)){
			$.message("查询语句不能包含‘<’或者‘>’符号！");
			return false;	
		}
		if(d.countsql && (d.countsql.indexOf(">") > -1 || d.countsql.indexOf("<") > -1)){
			$.message("计数语句不能包含‘<’或者‘>’符号！");
			return false;	
		} */
		d.mxkey = d.name;
		delete d.name;
...
function initfield(_this){
	...
	$.call("crdc.scheme.fields.list",{"name":d},function(data){
		...
		if(d){
			...
			tr.find("input[name='maxlength_show']").val(d.maxlength_show);
