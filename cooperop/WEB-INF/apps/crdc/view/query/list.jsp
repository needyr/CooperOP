...
<s:page title="单据设计" >
	<s:row>
		<s:form id="qform" fclass="portlet light bordered">
			...
			<s:row>
				<s:autocomplete label="查询方案标识" name="id" value="${tcdata.id }" id="autoquery"
	...
	<s:row>
		<s:table cols="4" label="查询方案列表" sort="false" action="crdc.designer.query" autoload="false" id="dtable" select="multi">
			<s:toolbar>
				<s:button label="导入查询" onclick="upfile();"></s:button>
				<s:button label="导出查询" onclick="downfile();"></s:button>
				<s:button label="新增查询" onclick="adddj();"></s:button>
			...
			<s:table.fields>
				...
				<s:table.field name="id" label="查询方案标识"></s:table.field>
				<s:table.field name="description" label="查询描述"></s:table.field>
...
$(document).ready(function(){
	query();
	$("#autoquery").params({type : "query"});
})
	function adddj() {
		$.openTabPage("new_chart", "新增查询", "crdc.query.modify", {_pid_: '${param._pid_}'});
	}
	function modifydj(type, id, flag,system_product_code ) {
		$.openTabPage(system_product_code+"query" + id, system_product_code+"_"+id, "crdc.query.modify", {schemeid: id,
			flag: flag,
			type: type,
			_pid_: '${param._pid_}',
	...
	function upfile(){
		$.modal(cooperopcontextpath + "/w/crdc/import_file.html", "导入查询", {
			width: '400px',
			height: '400px',
			callback: function(r){
			if(r){
