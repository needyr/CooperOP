...
<s:page ismodal="true" title="">
	<s:row>
		<s:form id="setFrom" fclass="portlet light bordered">
			...
			<s:row>
				<s:textfield label="显示字段" name="v1" value="${control.v1 }"></s:textfield>
				<s:textfield label="存储字段" name="v2" value="${control.v2 }"></s:textfield>
			</s:row>
			<c:if test="${empty control.create_action }">
...
<script>
	function save() {
		...
		data.attrs = attrs;
		data.children = [];
		if(attrs.enter_action){
			var child = {label:　'$['+attrs.v1+']', value: 　'$['+attrs.v2+']', contentIndex: 0};
