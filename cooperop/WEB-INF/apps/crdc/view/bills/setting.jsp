...
<s:page ismodal="true" title="">
	<s:row>
		<s:form id="setFrom" fclass="portlet light bordered">
			...
			<s:row>
				<s:textfield label="字段类型" name="fdtype" value="${control.fdtype }" required="true"></s:textfield>
				<s:textfield label="字段长度" name="size"
					value="${control.maxlength }" required="true"></s:textfield>
			...
			<c:if test="${ftype eq 'file' or ftype eq 'image'}">
				<s:row>
					<s:switch label="可删除" name="deleteable" value="${empty control.deleteable?'true':control.deleteable }" onvalue="true" offvalue="false"></s:switch>
					<s:switch label="可新增" name="addable" value="${empty control.addable?'true':control.addable }" onvalue="true" offvalue="false"></s:switch>
					<s:switch label="可下载" name="downloadable" value="${empty control.downloadable?'true':control.downloadable }" onvalue="true" offvalue="false"></s:switch>
				</s:row>
			</c:if>
			...
			<c:if test="${ftype eq 'autocomplete' }">
				<s:row>
					...
					<s:switch label="默认为登陆人" name="autoinit" onvalue="Y" offvalue="N" value="${control.autoinit}">
					</s:switch>
				</s:row>
			</c:if>
			<s:row  >
