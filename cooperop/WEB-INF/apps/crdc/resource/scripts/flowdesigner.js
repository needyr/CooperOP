$.fn.extend({
	"flowdesigner" : function() {
		...
		$this.save = function() {
			...
			$.call("crdc.workflowdesigner.save", {"wfjson": $.toJSON(data)}, function(rtn) {
				$.success("保存成功！");
			});
		}
		
		$this.load = function(system_product_code, id) {
			$.call("crdc.workflowdesigner.get", {"system_product_code": system_product_code, id: id}, function(rtn) {
		...
		$this.deploy = function() {
			...
			$.call("crdc.workflowdesigner.deploy", {"wfjson": $.toJSON(data)}, function(rtn) {
				$.success("发布成功！");
			});
		}
		...
		$this.onContextMenuClick = function(menu) {
			if (context_menu[0].menuObject instanceof Flow) {
				if (menu == "flowproperty") {
					...
					$.modal(cooperopcontextpath + "/w/crdc/workflow/flow.html", "流程属性", $.extend(true, data, {
						//width: "640px",
						//height: "480px",
				...
				if (context_menu[0].menuObject.type.id != "start" && context_menu[0].menuObject.type.id != "end" && menu == "nodeproperty") {
					...
					$.modal(cooperopcontextpath + "/w/crdc/workflow/node.html", "节点属性 [" + t.name + "]", $.extend(true, data, {
						type: t.type.id,
						//width: "640px",
						//height: "480px",
						callback: function(rtn) {
							if (rtn) {
								delete t.processor_role;
								delete t.processor_post;
								delete t.processors;
								delete t.assistant_role;
								delete t.assistants;
								
								delete t.processors_role_label;
								delete t.processors_post_label;
								delete t.processors_label;
								delete t.assistants_role_label;
								delete t.assistants_label;
								t = $.extend(true, t, rtn);
								$this.paint();
							}
						}
				...
				if (menu == "routeproperty") {
					...
					$.modal(cooperopcontextpath + "/w/crdc/workflow/route.html", "流向属性 [" + t.fromNode.name + " -> " + t.toNode.name + "]", $.extend(true, data, {
						//width: "640px",
						//height: "480px",
						callback: function(rtn) {
							if (rtn) {
