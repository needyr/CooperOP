...
//				ondragover:"allowDrop(event)"	
			...
			cc = t.append(dtype, {
				...
				color:"grey-silver",
				draggable:"true",
				ondragstart:"dragChart(event, this)",
				ondrop:"dropChart(this)",
				ondragover:"allowDrop(event)"
			});
		}else if(dtype=="chart1"){
			var t = designer.page.get(divdom);
			cc = t.append("chart", {
				label:"仪表盘",cols:"2",
				chart_type: '3',
		...
		var d = $(divdom);
		if($(srcRowdom).hasClass("row-fluid") && $(divdom).hasClass("row-fluid")
...
function dropChart(divdom) {
	if(ptype != ""){//表示从左边拖过来新增的
		
	}else{
		var d = $(divdom);
		if($(srcChartdom).hasClass("row-fluid") && $(divdom).hasClass("row-fluid")
			&& $(srcChartdom).attr("type")!="frow" && $(divdom).attr("type")!="frow"){
			$(srcChartdom).insertBefore(divdom);
			moveContents(srcChartdom,divdom,"insert");
		}
	}
	dragclear();
}
