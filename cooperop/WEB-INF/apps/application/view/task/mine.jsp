...
<s:page title="我的任务列表" dispermission="true">
 	<s:row>
		<s:tabpanel>
			<s:form label="待办事项" icon="fa fa-calendar" active="true">
				<s:row>
				...
				<s:row>
					<s:table id="optionlist" action="application.task.queryMine" autoload="false" fitwidth="true" sort="true" label="任务列表">
						<s:table.fields>
							...
							<s:table.field name="create_time" label="到达时间" align="center" sort="true" width="60"></s:table.field>
							...
							<s:table.field name="oper" datatype="script" label="操作"
								...
								if (record.is_cc == 1) {
									...
									html.push('\'' + record.task_id + '\',\'' + record.order_id + '\',\'' + record.node_name + '\',\'' + record.wx_auth + '\')">处理</a>');
								}
								return html.join("");
							</s:table.field>
						</s:table.fields>
			...
			<s:form label="我发起的" icon="fa fa-tasks">
				<s:row>
					<s:form id="conditions2" fclass="portlet light bordered">
						...
						<s:row>
							...
							<s:button label="查询" icon="fa fa-search" color="green" onclick="query2()"></s:button>
						</s:row>
					</s:form>
				</s:row>
				<s:row>
					<s:table id="optionlist2" action="application.task.queryTasking" autoload="false" fitwidth="true" sort="true" label="任务列表">
						<s:table.fields>
							...
							<s:table.field name="state" label="当前状态" width="80" datatype="script">
								...
								if (record.order_end_time) {
									html.push("已办结 ");
									if(record.audited == 'N'){
										html.push("<font color=\"red\"> [驳回]</font>");
									}else{
										html.push(" [通过]");
									}
									...
									if(record.is_skim_time){
										html.push(record.node_name+" <font color=\"blue\"> [处理中]</font>");
									}else{
										html.push(record.node_name+"<font color=\"#409c58\"> [待处理]</font>");
									}
								}
								return html.join("");
							</s:table.field>
			...
			<s:form label="我经办的" icon="fa fa-check-square-o">
				<s:row>
					<s:form id="conditions3" fclass="portlet light bordered">
						...
						<s:row>
							...
							<s:button label="查询" icon="fa fa-search" color="green" onclick="query3()"></s:button>
						</s:row>
					</s:form>
				</s:row>
				<s:row>
					<s:table id="optionlist3" action="application.task.queryHistory" autoload="false" fitwidth="true" sort="true" label="任务列表">
						<s:table.fields>
							<s:table.field name="system_product_name" label="所属产品"  sort="true" width="80" app_field="state_field"></s:table.field>
							<s:table.field name="order_no" label="单据号"  sort="true" width="120" app_field="state_field"></s:table.field>
							<s:table.field name="subject" label="主题"  datatype="script" sort="true" app_field="title_field">
								var html = [];
								html.push('<a style="margin: 0px 5px;" href="javascript:void(0)" ');
								html.push('onclick="todeal(\'' + record.info_bill + '\', \'' + record.order_no + '\', ');
							...
							<s:table.field name="node_name" label="处理环节" width="100" datatype="script" app_field="pre_title_field">
								var html=[];
							...
							<s:table.field name="state" label="当前状态" width="80" datatype="script">
								...
								if (record.order_end_time) {
									...
									html.push(record.current_node_name);
									if(record.is_skim_time){
										html.push(record.node_name+" <font color=\"blue\"> [处理中]</font>");
									}else{
										html.push(record.node_name+"<font color=\"#409c58\"> [待处理]</font>");
...
<script type="text/javascript">
	...
	function to_deal(pageid,order_no,task_id,order_id,node_name,wx_auth){
		$.call("application.task.skimsave",{task_id: task_id}, function(rtn){
		});
		
		todeal(pageid,order_no,task_id,order_id,node_name,wx_auth);
	}
	function todeal(pageid,order_no,task_id,order_id,node_name,wx_auth){
		var u = pageid.split(".").join("/");
		var url = cooperopcontextpath + "/w/" + u + ".html";
		if(wx_auth == 'Y'){
			url = cooperopcontextpath + "/w/application/task/wxauth.html";
		}
		if(pageid.indexOf("bmc")>=0){
			url = cooperopcontextpath +"/w/"+ pageid;
		}
		if(url.indexOf('/w/bmc.ssc.pm.user.task_detail.html') >= 0){
			$.message("此任务只能在bmc人力资源管理模块中的tph端口点击处理！");
		}else if(url.indexOf('/w/bmc.ssc.pm.user.task_modify.html') >= 0){
			$.message("此任务只能在bmc人力资源管理模块中的tph端口点击处理！");
		}else{
			var wei = '99%';
			var hei = '99%';
