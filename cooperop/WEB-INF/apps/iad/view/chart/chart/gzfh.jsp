...
<s:page ismodal="false" disloggedin="false" dispermission="false" schemeid="gzfh" init-action="" description="运营简讯（外系科室）-工作负荷" system_product_code="iad" title="" remark="" flag="chart" type="chart"><%
	Map<String, Object> a = null;
	if(!CommonFun.isNe(request.getParameter("djbh"))){
		a = BillAction.init("iad.chart.chart.gzfh", (String)request.getParameter("djbh"));
	} else if(!CommonFun.isNe(request.getParameter("gzid"))){
		a = BillAction.initFromCache("iad.chart.chart.gzfh", (String)request.getParameter("gzid"));
	} else if(!CommonFun.isNe(request.getParameter("p_dj_sn"))){
		a = BillAction.initFromCacheMX(request.getParameter("p_pageid"), "iad.chart.chart.gzfh", (String)request.getParameter("ptableid"), (String)request.getParameter("p_dj_sn"), (String)request.getParameter("p_dj_sort"), (String)request.getParameter("p_gzid"));
	} else {
		a = BillAction.init("iad.chart.chart.gzfh", null);
	}
...
<s:textfield label="科室：" cols="1" name="visit_dept" placeholder="" fdtype="字符" defaultValue="" format="" nextfocusfield="" islabel="否" readonly="false" required="否" encryption="否" create_action="" modify_action="" enter_action="" dbl_action="zl_select_gzfh_01" out_action="" tableid="" expressions="" maxlength="32" precision="">${empty visit_dept ? '' : visit_dept}</s:textfield>
</s:row>
<s:toolbar><s:button label="查询" icon="glyphicon glyphicon-search" nextfocusfield="" action="querymx" color=""></s:button>
</s:toolbar>
</s:form>
</s:row>
<s:row><s:chart label="工作负荷 门诊人次" cols="1" is_span="Y" color="grey-silver" chart_flag="fh" chart_height="" sub_label="更多" sub_label_href="iad.chart.chart.gzfh_1" span_head=" 门诊人次" span_content=":rc" span_footer="本期同比增幅  :zfl" welcome_func="N" autoload="true" autorefresh="Y" refresh_time="" flag="fh"></s:chart>
<s:chart label="工作负荷 急诊人次" cols="1" is_span="Y" color="grey-silver" chart_flag="fh2" chart_height="" sub_label="更多" sub_label_href="iad.chart.chart.gzfh_1" span_head="急诊人次" span_content=":rc" span_footer="本期同比增幅   :zfl" welcome_func="N" autoload="true" autorefresh="Y" refresh_time="" flag="fh2"></s:chart>
</s:row>
<s:row><s:chart label="门诊人次排名前5名科室" cols="2" color="grey-silver" chart_flag="fh3" tableid="fh3" chart_height="650" drill_chart="" drill_chart_name="" autoload="true" welcome_func="N" group_field="type" g_name="visit_dept" sub_label="更多" sub_label_href="iad.chart.chart.gzfh_2" flag="fh3" yaxis="[{name=je, label=金额, chart_type=bar}]"></s:chart>
<s:chart label="急诊人次排名前5科室" cols="2" color="grey-silver" chart_flag="fh4" tableid="fh4" chart_height="650" drill_chart="" drill_chart_name="" autoload="true" welcome_func="N" group_field="type" g_name="visit_dept" sub_label="更多" sub_label_href="iad.chart.chart.gzfh_2" flag="fh4" yaxis="[{name=je, label=金额, chart_type=bar}]"></s:chart>
</s:row>
</s:page>
