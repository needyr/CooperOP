$.fn.extend({
	"ccinit_table" : function() {
		...
		$this.DataTable({
			...
			"fnServerData" : function(sSource, aoData, fnCallback) {
				...
				if($this.attr("totals")){
					var totalsJson = eval('(' + $this.attr("totals") + ')');
					data["totals"] = totalsJson.params;
				}else{
				...
				$.call(action, data, function(rs) {
					...
					if($this.attr("totals")){
						var totalsJson = eval('(' + $this.attr("totals") + ')');
						var info = totalsJson.info;
						var to_p = totalsJson.params;
						if (rs.totals) {
							var t = to_p.split(",");
							var rrtotal = rs.totals;
							for (var i = 1; i < t.length; i += 2) {
								info = info.replace(":"+t[i], "<span>"+(rrtotal[t[i]] || "0") +"</span>");
							}
						}
						$(".table-info .table-totals"+($this.attr("tableid")?"."+$this.attr("tableid"):"")).html(info);
					}
					j["aaData"] = _rdata;
					fnCallback(j);
					$("[expressions][expressions!='']").each(function() {
	...
	"setting_table" : function() {
		...
		html.push('		<h3 style="margin-top: 0px;">显示/隐藏列<em class="dataTables_columns_tips">(按住鼠标左键可拖动改变列显示顺序)</em></h3>');
		html.push('		<ul>');
