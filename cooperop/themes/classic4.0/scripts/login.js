$(document).ready(function() {
	...
	$(".others ul .account-div a").on("click", function(){
		$("#loginForm .logindiv").hide();
		$("#loginForm").find(".forms").show();
		$(".others ul li").show();
		$(this).parents(".account-div").hide();
		cancelfinger();
	});
	$(".others ul .weixin-div a").on("click", function(){
		$("#loginForm .logindiv").hide();
		$("#loginForm").find(".qr-code").show();
		$(".others ul li").show();
		...
		$(this).parents(".weixin-div").hide();
		cancelfinger();
	});
	$(".others ul .zhiwen-div a").on("click", function(){
		$("#loginForm .logindiv").hide();
		$("#loginForm").find(".finger").show();
		$(".others ul li").show();
		$(this).parents(".zhiwen-div").hide();
		usefinger();
	});
	$(".qr-code .btn-close").on("click", function(){
		$(".others ul .account-div").find("a")[0].click();
	});
});
	...
	$.loginfinger({callback: function(rtn){
		if(rtn.redirect_url){
			...
			$.message("验证失败，请使用账号登陆！", function(){
				$(".others ul").find(".account-div a").trigger("click");
			});
		}
	}
	});
