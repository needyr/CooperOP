...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template name="cooperop-toolbar-link">
		<xsl:if test="descendant::cooperop-toolbar">
			<link href="{$contextpath}/theme/css/components/layout/toolbar.css?_v={$_v}" rel="stylesheet" type="text/css" />
			<script type="text/javascript" src="{$contextpath}/theme/scripts/controls/layout/toolbar.js?_v={$_v}"></script>
		</xsl:if>
	</xsl:template>
	<xsl:template match="cooperop-toolbar">
		<xsl:param name="active"></xsl:param>
		<xsl:choose>
			<xsl:when test="parent::cooperop-form or parent::cooperop-table or parent::cooperop-list or @tooltype='tftool'">
