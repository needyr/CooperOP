	...
	<xsl:template name="cooperop-select-link">
		<xsl:if test="descendant::cooperop-select">
			...
			<script type="text/javascript"
				src="{$contextpath}/theme/scripts/controls/input/select.js?_v={$_v}"></script>
		</xsl:if>
	</xsl:template>
			...
			<xsl:choose>
				<xsl:when test="parent::cooperop-tablefield or parent::cooperop-tabledata">
					<div class="control-content">
						...
						<span class="select-tip"><i class="cicon icon-dropdown2"></i></span>
					</div>
				</xsl:when>
				<xsl:otherwise>
					<div class="cols{@cols}" crid="{@crid}">
						...
						<div class="control-content">
							<xsl:choose>
								...
								<xsl:otherwise>
									<select ctype="select" class="form-control " value="{@defaultValue}">
										<xsl:if test="@required ='true' or @required ='required'">
											<xsl:attribute name="required">required</xsl:attribute>
										</xsl:if>
										<xsl:if test="@readonly ='true'">
