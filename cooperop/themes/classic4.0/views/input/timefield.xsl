...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="cooperop-timefield-link">
		<xsl:if test="descendant::cooperop-timefield">
			<link rel="stylesheet" type="text/css" href="{$contextpath}/theme/plugins/bootstrap-timepicker/css/bootstrap-timepicker.min.css?_v={$_v}"/>
			<link href="{$contextpath}/theme/css/components/input/timefield.css?_v={$_v}" rel="stylesheet" type="text/css" /> 
			<script type="text/javascript" src="{$contextpath}/theme/scripts/controls/input/timefield.js?_v={$_v}"></script>
			<script type="text/javascript" src="{$contextpath}/theme/plugins/bootstrap-timepicker/js/bootstrap-timepicker.min.js?_v={$_v}"></script>
		</xsl:if>
	</xsl:template>
	<xsl:template match="cooperop-timefield">
		<xsl:choose>
			<xsl:when test="parent::cooperop-tablefield or parent::cooperop-tabledata">
				<div class="control-content">
					...
					<i class="control-icon cicon icon-time"></i>
					<input ctype="timefield" class="form-control time-picker" type="text" name="{@name}" value="">
						<xsl:if test="@required ='true' or @required ='required'">
