...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="cooperop-select2-link">
		<xsl:if test="descendant::cooperop-select2">
			<link href="{$contextpath}/theme/plugins/select2/css/select2.min.css" type="text/css" rel="stylesheet" />
			<link href="{$contextpath}/theme/css/components/input/select2.css?_v={$_v}" rel="stylesheet" type="text/css" />
			<script type="text/javascript" src="{$contextpath}/theme/plugins/select2/select2.min.js"></script>
			<script type="text/javascript" src="{$contextpath}/theme/plugins/select2/i18n/zh-CN.js"></script>
			<script type="text/javascript" src="{$contextpath}/theme/scripts/controls/input/select2.js?_v={$_v}"></script>
		</xsl:if>
	</xsl:template>
	<xsl:template match="cooperop-select2">
		<xsl:if test="child::cooperop-option">
			<div class="cols{@cols}" crid="{@crid}">
				...
				<div class="control-content select2 has-icon">
					<xsl:choose>
						...
						<xsl:otherwise>
							<select ctype="select2" class="form-control " value="{@defaultValue}" multiple="multiple">
								<xsl:if test="@required ='true' or @required ='required'">
