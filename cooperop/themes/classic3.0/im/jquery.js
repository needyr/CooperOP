/*
 * ! chohojQuery JavaScript Library v1.11.2 http://chohojquery.com/
 * 
 * Includes Sizzle.js http://sizzlejs.com/
 * 
 * Copyright 2005, 2014 chohojQuery Foundation, Inc. and other contributors
 * Released under the MIT license http://chohojquery.org/license
 * 
...
(function(global, factory) {
	...
	if (typeof module === "object" && typeof module.exports === "object") {
		...
		// present,
		// execute the factory and get chohojQuery
		// For environments that do not inherently posses a window with a
		// document
		// (such as Node.js), expose a chohojQuery-making factory as
		// module.exports
		// This accentuates the need for the creation of a real window
		// e.g. var chohojQuery = require("chohojquery")(window);
		// See ticket #14549 for more info
		module.exports = global.document ? factory(global, true)
				: function(w) {
					if (!w.document) {
						throw new Error(
								"chohojQuery requires a window with a document");
					}
					return factory(w);
				};
	} else {
	...
	// Pass this if window is not defined yet
		...
		(
				...
				function(window, noGlobal) {
					...
					// Define a local copy of chohojQuery
					chohojQuery = function(selector, context) {
						// The chohojQuery object is actually just the init
						// constructor 'enhanced'
						// Need init if chohojQuery is called (just allow error
						// to
						// be thrown if not included)
						return new chohojQuery.fn.init(selector, context);
					},
					...
					// Used by chohojQuery.camelCase as callback to replace()
					fcamelCase = function(all, letter) {
						return letter.toUpperCase();
					};

					chohojQuery.fn = chohojQuery.prototype = {
						// The current version of chohojQuery being used
						chohojquery : version,

						constructor : chohojQuery,

						// Start with an empty selector
						selector : "",

						// The default length of a chohojQuery object is 0
						length : 0,
						...
						pushStack : function(elems) {
							...
							// Build a new chohojQuery matched element set
							var ret = chohojQuery.merge(this.constructor(),
									elems);
						...
						each : function(callback, args) {
							return chohojQuery.each(this, callback, args);
						},

						map : function(callback) {
							return this.pushStack(chohojQuery.map(this,
									function(elem, i) {
						...
						// Behaves like an Array's method, not like a
						// chohojQuery
						// method.
						push : push,
					...
					chohojQuery.extend = chohojQuery.fn.extend = function() {
						...
						if (typeof target !== "object"
								&& !chohojQuery.isFunction(target)) {
							target = {};
						}

						// extend chohojQuery itself if only one argument is
						// passed
						if (i === length) {
							target = this;
							i--;
						...
						for (; i < length; i++) {
							...
							if ((options = arguments[i]) != null) {
								...
								for (name in options) {
									...
									if (deep
											&& copy
											&& (chohojQuery.isPlainObject(copy) || (copyIsArray = chohojQuery
													.isArray(copy)))) {
										if (copyIsArray) {
											copyIsArray = false;
											clone = src
													&& chohojQuery.isArray(src) ? src
													: [];
										...
										// them
										target[name] = chohojQuery.extend(deep,
												clone, copy);
					...
					chohojQuery
							.extend({
								// Unique for each copy of chohojQuery on the
								// page
								expando : "chohojQuery"
										+ (version + Math.random()).replace(
												/\D/g, ""),

								// Assume chohojQuery is ready without the ready
								// module
								isReady : true,
								...
								isFunction : function(obj) {
									return chohojQuery.type(obj) === "function";
								},

								isArray : Array.isArray || function(obj) {
									return chohojQuery.type(obj) === "array";
								},
								...
								isNumeric : function(obj) {
									...
									// parseFloat (#15100)
									return !chohojQuery.isArray(obj)
								...
								isPlainObject : function(obj) {
									...
									if (!obj
											|| chohojQuery.type(obj) !== "object"
											|| obj.nodeType
											|| chohojQuery.isWindow(obj)) {
										return false;
									}
								...
								// chohojQuery.support is not used in Core but
								// other projects attach their
					...
					function isArraylike(obj) {
						var length = obj.length, type = chohojQuery.type(obj);

						if (type === "function" || chohojQuery.isWindow(obj)) {
							return false;
						}
					 ...
					 * http://sizzlejs.com/
					 * 
					 * Copyright 2008, 2014 chohojQuery Foundation, Inc. and
					 * other contributors Released under the MIT license
					 * http://chohojquery.org/license
					 * 
					 * Date: 2014-12-16
					 */
					(function(window) {
						...
						function Sizzle(selector, context, results, seed) {
							...
							if (!seed && documentIsHTML) {
								...
								// Try to shortcut find operations when possible
								// (e.g., not under DocumentFragment)
								if (nodeType !== 11
						...
						setDocument = Sizzle.setDocument = function(node) {
							...
							// accessing "document" variable, see chohojQuery
							// #13936
							...
							// See http://bugs.chohojquery.com/ticket/13378
							rbuggyQSA = [];
							...
							if ((support.qsa = rnative
								...
								assert(function(div) {
									...
									// http://bugs.chohojquery.com/ticket/12359
									...
									// Support: Chrome<29, Android<4.2+,
									// Safari<7.0+, iOS<7.0+, PhantomJS<1.9.7+
						...
						Sizzle.uniqueSort = function(results) {
							...
							// See
							// https://github.com/chohojquery/sizzle/pull/225
							sortInput = null;
					...
					chohojQuery.find = Sizzle;
					chohojQuery.expr = Sizzle.selectors;
					chohojQuery.expr[":"] = chohojQuery.expr.pseudos;
					chohojQuery.unique = Sizzle.uniqueSort;
					chohojQuery.text = Sizzle.getText;
					chohojQuery.isXMLDoc = Sizzle.isXML;
					chohojQuery.contains = Sizzle.contains;

					var rneedsContext = chohojQuery.expr.match.needsContext;
					...
					function winnow(elements, qualifier, not) {
						if (chohojQuery.isFunction(qualifier)) {
							return chohojQuery
									.grep(elements,
											function(elem, i) {
												/* jshint -W018 */
						...
						if (qualifier.nodeType) {
							return chohojQuery.grep(elements, function(elem) {
						...
						if (typeof qualifier === "string") {
							if (risSimple.test(qualifier)) {
								return chohojQuery.filter(qualifier, elements,
										not);
							}

							qualifier = chohojQuery.filter(qualifier, elements);
						}

						return chohojQuery
								.grep(elements,
										function(elem) {
											return (chohojQuery.inArray(elem,
													qualifier) >= 0) !== not;
										});
					}

					chohojQuery.filter = function(expr, elems, not) {
						var elem = elems[0];
						...
						return elems.length === 1 && elem.nodeType === 1 ? chohojQuery.find
								.matchesSelector(elem, expr) ? [ elem ] : []
								: chohojQuery.find.matches(expr, chohojQuery
							...
							.extend({
								find : function(selector) {
									...
									if (typeof selector !== "string") {
										return this
												.pushStack(chohojQuery(selector)
														.filter(
																function() {
																	for (i = 0; i < len; i++) {
																		if (chohojQuery
									...
									ret = this.pushStack(len > 1 ? chohojQuery
											.unique(ret) : ret);
								...
								is : function(selector) {
									return !!winnow(
											...
											typeof selector === "string"
													&& rneedsContext
															.test(selector) ? chohojQuery(selector)
													: selector || [], false).length;
								}
							});

					// Initialize a chohojQuery object

					// A central reference to the root chohojQuery(document)
					var rootjQuery,
					...
					init = chohojQuery.fn.init = function(selector, context) {
						var match, elem;
						...
						if (typeof selector === "string") {
							...
							if (match && (match[1] || !context)) {
								...
								// HANDLE: $(html) -> $(array)
								if (match[1]) {
									context = context instanceof chohojQuery ? context[0]
											: context;
									...
									if (elem && elem.parentNode) {
										...
										// directly into the chohojQuery object
										this.length = 1;
										this[0] = elem;
									}
								...
								// HANDLE: $(expr, $(...))
							} else if (!context || context.chohojquery) {
						...
						return chohojQuery.makeArray(selector, this);
					};

					// Give the init function the chohojQuery prototype for
					// later
					// instantiation
					init.prototype = chohojQuery.fn;

					// Initialize central reference
					rootjQuery = chohojQuery(document);
					...
					chohojQuery.fn
							.extend({
								has : function(target) {
									var i, targets = chohojQuery(target, this), len = targets.length;

									return this.filter(function() {
										for (i = 0; i < len; i++) {
											if (chohojQuery.contains(this,
													targets[i])) {
												return true;
											}
										}
								...
								closest : function(selectors, context) {
									var cur, i = 0, l = this.length, matched = [], pos = rneedsContext
											.test(selectors)
											|| typeof selectors !== "string" ? chohojQuery(
									...
									return this
											.pushStack(matched.length > 1 ? chohojQuery
													.unique(matched)
													: matched);
								},
								...
								index : function(elem) {
									...
									if (typeof elem === "string") {
										return chohojQuery.inArray(this[0],
												chohojQuery(elem));
									}

									// Locate the position of the desired
									// element
									return chohojQuery.inArray(
									// If it receives a chohojQuery object, the
									// first element is used
									elem.chohojquery ? elem[0] : elem, this);
								},

								add : function(selector, context) {
									return this.pushStack(chohojQuery
											.unique(chohojQuery.merge(this
													.get(), chohojQuery(
													selector, context))));
								},
							...
							.each(
									{
										...
										contents : function(elem) {
											return chohojQuery.nodeName(elem,
									...
									function(name, fn) {
										chohojQuery.fn[name] = function(until,
												selector) {
											var ret = chohojQuery.map(this, fn,
													until);
					...
					function createOptions(options) {
						...
						chohojQuery.each(options.match(rnotwhite) || [],
								function(_, flag) {
									object[flag] = true;
								});
						return object;
					...
					chohojQuery.Callbacks = function(options) {
						...
						self = {
							...
							has : function(fn) {
								return fn ? chohojQuery.inArray(fn, list) > -1
										: !!(list && list.length);
							},
								...
								Deferred : function(func) {
									var tuples = [
											...
											[
													...
													"progress",
													chohojQuery
															.Callbacks("memory") ] ], state = "pending", promise = {
										state : function() {
											return state;
										},
										always : function() {
					...
					chohojQuery.fn.ready = function(fn) {
						// Add the callback
						chohojQuery.ready.promise().done(fn);
					...
					chohojQuery.extend({
						...
						holdReady : function(hold) {
							if (hold) {
								chohojQuery.readyWait++;
							} else {
								chohojQuery.ready(true);
							}
						},
						...
						ready : function(wait) {
							...
							// ready
							if (wait === true ? --chohojQuery.readyWait
									: chohojQuery.isReady) {
								return;
							}
							...
							if (!document.body) {
								return setTimeout(chohojQuery.ready);
							}

							// Remember that the DOM is ready
							chohojQuery.isReady = true;

							// If a normal DOM Ready event fired, decrement, and
							// wait if need be
							if (wait !== true && --chohojQuery.readyWait > 0) {
								return;
							}

							// If there are functions bound, to execute
							readyList.resolveWith(document, [ chohojQuery ]);

							// Trigger any bound ready events
							if (chohojQuery.fn.triggerHandler) {
								chohojQuery(document).triggerHandler("ready");
								chohojQuery(document).off("ready");
							}
						}
					});
					...
					chohojQuery.ready.promise = function(obj) {
						if (!readyList) {

							readyList = chohojQuery.Deferred();
							...
							// http://bugs.chohojquery.com/ticket/12282#comment:15
							if (document.readyState === "complete") {
								// Handle it asynchronously to allow scripts the
								// opportunity to delay ready
								setTimeout(chohojQuery.ready);
					 ...
					 */
					chohojQuery.acceptData = function(elem) {
						var noData = chohojQuery.noData[(elem.nodeName + " ")
					...
					function dataAttr(elem, key, data) {
						...
						if (data === undefined && elem.nodeType === 1) {
							...
							if (typeof data === "string") {
								...
								// later
								chohojQuery.data(elem, key, data);
					...
					function isEmptyDataObject(obj) {
						...
						for (name in obj) {
							...
							// is still empty
							if (name === "data"
									&& chohojQuery.isEmptyObject(obj[name])) {
								continue;
							}
							if (name !== "toJSON") {
								return false;
					...
					function internalData(elem, name, data, pvt /*
																 ...
																 * Only
																 */) {
						if (!chohojQuery.acceptData(elem)) {
							return;
						}

						var ret, thisCache, internalKey = chohojQuery.expando,
						...
						// Only DOM nodes need the global chohojQuery cache; JS
						// object data is
						// attached directly to the object so GC can occur
						// automatically
						cache = isNode ? chohojQuery.cache : elem,
						...
						if (!cache[id]) {
							// Avoid exposing chohojQuery metadata on plain JS
							// objects when the object
							// is serialized using JSON.stringify
							cache[id] = isNode ? {} : {
								toJSON : chohojQuery.noop
							};
						}

						// An object can be passed to chohojQuery.data instead
						// of
						// a key/value pair; this gets
						// shallow copied over onto the existing cache
						if (typeof name === "object"
								|| typeof name === "function") {
							if (pvt) {
								cache[id] = chohojQuery.extend(cache[id], name);
							} else {
								cache[id].data = chohojQuery.extend(
										cache[id].data, name);
							}
						}
						...
						// chohojQuery data() is stored in a separate object
						// inside the object's internal data
						...
						if (data !== undefined) {
							thisCache[chohojQuery.camelCase(name)] = data;
						}
						...
						if (typeof name === "string") {
							...
							if (ret == null) {
								...
								ret = thisCache[chohojQuery.camelCase(name)];
							}
						} else {
							ret = thisCache;
						}
					...
					function internalRemoveData(elem, name, pvt) {
						...
						// See chohojQuery.data for more information
						cache = isNode ? chohojQuery.cache : elem, id = isNode ? elem[chohojQuery.expando]
								: chohojQuery.expando;
							...
							if (thisCache) {
								...
								if (!chohojQuery.isArray(name)) {
									...
									if (name in thisCache) {
										...
										// exists
										name = chohojQuery.camelCase(name);
										if (name in thisCache) {
											name = [ name ];
										} else {
											name = name.split(" ");
									...
									// argument path.
									name = name.concat(chohojQuery.map(name,
											chohojQuery.camelCase));
								}
								...
								if (pvt ? !isEmptyDataObject(thisCache)
										: !chohojQuery.isEmptyObject(thisCache)) {
									return;
								}
							}
						}
						...
						// Destroy the cache
						if (isNode) {
							chohojQuery.cleanData([ elem ], true);
					...
					chohojQuery
							.extend({
								...
								hasData : function(elem) {
									elem = elem.nodeType ? chohojQuery.cache[elem[chohojQuery.expando]]
											: elem[chohojQuery.expando];
					...
					chohojQuery.fn
							.extend({
								data : function(key, value) {
									...
									if (key === undefined) {
										if (this.length) {
											data = chohojQuery.data(elem);

											if (elem.nodeType === 1
													&& !chohojQuery._data(elem,
															"parsedAttrs")) {
												i = attrs.length;
												while (i--) {
									...
									// Sets one value
									this.each(function() {
										chohojQuery.data(this, key, value);
									}) :

									// Gets one value
									// Try to fetch any internally stored data
									// first
									elem ? dataAttr(elem, key, chohojQuery
											.data(elem, key)) : undefined;
								},

								removeData : function(key) {
									return this.each(function() {
										chohojQuery.removeData(this, key);
									});
								}
							});
					...
					chohojQuery
							.extend({
								queue : function(elem, type, data) {
									...
									if (elem) {
										type = (type || "fx") + "queue";
										queue = chohojQuery._data(elem, type);
								...
								dequeue : function(elem, type) {
									...
									var queue = chohojQuery.queue(elem, type), startLength = queue.length, fn = queue
											.shift(), hooks = chohojQuery
											._queueHooks(elem, type), next = function() {
										chohojQuery.dequeue(elem, type);
									};
								...
								_queueHooks : function(elem, type) {
									...
									return chohojQuery._data(elem, key)
											|| chohojQuery
													._data(
															elem,
															key,
							...
							.extend({
								queue : function(type, data) {
									...
									if (arguments.length < setter) {
										return chohojQuery.queue(this[0], type);
									}

									return data === undefined ? this
											: this
													.each(function() {
														var queue = chohojQuery
																.queue(this,
																		type,
																		data);
								...
								promise : function(type, obj) {
									var tmp, count = 1, defer = chohojQuery
									...
									while (i--) {
										tmp = chohojQuery._data(elements[i],
												type + "queueHooks");
										if (tmp && tmp.empty) {
											count++;
					...
					var isHidden = function(elem, el) {
						// isHidden might be called from chohojQuery#filter
						// function;
						// in that case, element will be second argument
						elem = el || elem;
						return chohojQuery.css(elem, "display") === "none"
								|| !chohojQuery.contains(elem.ownerDocument,
										elem);
					};
					...
					var access = chohojQuery.access = function(elems, fn, key,
							value, chainable, emptyGet, raw) {
						var i = 0, length = elems.length, bulk = key == null;

						// Sets many values
						if (chohojQuery.type(key) === "object") {
							chainable = true;
							for (i in key) {
								chohojQuery.access(elems, fn, i, key[i], true,
										emptyGet, raw);
							}
							...
							if (!chohojQuery.isFunction(value)) {
								raw = true;
							}
							...
							if (bulk) {
								...
								if (raw) {
									...
									fn = function(elem, key, value) {
										return bulk.call(chohojQuery(elem),
												value);
									};
								}
							}
					...
					chohojQuery.event = {
						...
						add : function(elem, types, handler, data, selector) {
							var tmp, events, t, handleObjIn, special, eventHandle, handleObj, handlers, type, namespaces, origType, elemData = chohojQuery
									._data(elem);
							...
							if (!(eventHandle = elemData.handle)) {
								eventHandle = elemData.handle = function(e) {
									...
									// chohojQuery.event.trigger() and
									// when an event is called after a page has
									// unloaded
									return typeof chohojQuery !== strundefined
											&& (!e || chohojQuery.event.triggered !== e.type) ? chohojQuery.event.dispatch
							...
							while (t--) {
								...
								special = chohojQuery.event.special[type] || {};
										...
										.extend(
												{
													...
													needsContext : selector
															&& chohojQuery.expr.match.needsContext
																	.test(selector),
								...
								chohojQuery.event.global[type] = true;
							}
						...
						remove : function(elem, types, handler, selector,
								mappedTypes) {
							var j, handleObj, tmp, origCount, t, events, special, handlers, type, namespaces, origType, elemData = chohojQuery
									.hasData(elem)
									&& chohojQuery._data(elem);
							...
							while (t--) {
								...
								if (!type) {
									for (type in events) {
										chohojQuery.event.remove(elem, type
								...
								if (origCount && !handlers.length) {
									if (!special.teardown
										...
										chohojQuery.removeEvent(elem, type,
												elemData.handle);
									}
						...
						trigger : function(event, data, elem, onlyHandlers) {
							...
							// Caller can pass in a chohojQuery.Event object,
							// Object, or just an event type string
							event = event[chohojQuery.expando] ? event
									: new chohojQuery.Event(type,
											typeof event === "object" && event);

							// Trigger bitmask: & 1 for native handlers; & 2 for
							// chohojQuery (always true)
							...
							data = data == null ? [ event ] : chohojQuery
							...
							while ((cur = eventPath[i++])
								...
								// chohojQuery handler
								handle = (chohojQuery._data(cur, "events") || {})[event.type]
										&& chohojQuery._data(cur, "handle");
								if (handle) {
							...
							if (!onlyHandlers && !event.isDefaultPrevented()) {
								...
								if ((!special._default || special._default
									...
									if (ontype && elem[type]
										...
										// above
										chohojQuery.event.triggered = type;
										try {
											elem[type]();
										} catch (e) {
										...
										chohojQuery.event.triggered = undefined;
						...
						dispatch : function(event) {
							...
							// Make a writable chohojQuery.Event from the native
							// event object
							event = chohojQuery.event.fix(event);

							var i, ret, handleObj, matched, j, handlerQueue = [], args = slice
									.call(arguments), handlers = (chohojQuery
									._data(this, "events") || {})[event.type]
									|| [], special = chohojQuery.event.special[event.type]
									|| {};

							// Use the fix-ed chohojQuery.Event rather than the
							...
							// Determine handlers
							handlerQueue = chohojQuery.event.handlers.call(
									this, event, handlers);
							...
							while ((matched = handlerQueue[i++])
								...
								while ((handleObj = matched.handlers[j++])
									...
									if (!event.namespace_re
										...
										ret = ((chohojQuery.event.special[handleObj.origType] || {}).handle || handleObj.handler)
									...
									if (cur.nodeType === 1
										...
										for (i = 0; i < delegateCount; i++) {
											...
											if (matches[sel] === undefined) {
												matches[sel] = handleObj.needsContext ? chohojQuery(
														sel, this).index(cur) >= 0
														: chohojQuery.find(sel,
																this, null,
																[ cur ]).length;
											}
											if (matches[sel]) {
						...
						fix : function(event) {
							...
							event = new chohojQuery.Event(originalEvent);
						...
						special : {
							...
							click : {
								...
								// state will be right
								trigger : function() {
									if (chohojQuery.nodeName(this, "input")
											&& this.type === "checkbox"
											&& this.click) {
										this.click();
										return false;
								...
								_default : function(event) {
									return chohojQuery.nodeName(event.target,
											"a");
								}
							},
						...
						simulate : function(type, elem, event, bubble) {
							...
							// same on the donor.
							var e = chohojQuery.extend(new chohojQuery.Event(),
									event, {
										type : type,
										isSimulated : true,
										originalEvent : {}
							...
							if (bubble) {
								chohojQuery.event.trigger(e, null, elem);
							} else {
								chohojQuery.event.dispatch.call(elem, e);
							}
					...
					chohojQuery.removeEvent = document.removeEventListener ? function(
							elem, type, handle) {
					...
					chohojQuery.Event = function(src, props) {
						// Allow instantiation without the 'new' keyword
						if (!(this instanceof chohojQuery.Event)) {
							return new chohojQuery.Event(src, props);
						}
						...
						// Mark it as fixed
						this[chohojQuery.expando] = true;
					};

					// chohojQuery.Event is based on DOM3 Events as specified by
					// the ECMAScript Language Binding
					// http://www.w3.org/TR/2003/WD-DOM-Level-3-Events-20030331/ecma-script-binding.html
					chohojQuery.Event.prototype = {
					...
					chohojQuery
							.each(
									...
									function(orig, fix) {
										chohojQuery.event.special[orig] = {
											delegateType : fix,
											bindType : fix,
					...
					if (!support.submitBubbles) {
						...
						chohojQuery.event.special.submit = {
							setup : function() {
								// Only need this for delegated form submit
								// events
								if (chohojQuery.nodeName(this, "form")) {
									return false;
								}
								...
								chohojQuery.event
										.add(
												...
												function(e) {
													...
													// (#9807)
													var elem = e.target, form = chohojQuery
															.nodeName(elem,
																	"input")
															|| chohojQuery
																	.nodeName(
							...
							postDispatch : function(event) {
								...
								if (event._submit_bubble) {
									...
									if (this.parentNode && !event.isTrigger) {
										chohojQuery.event.simulate("submit",
							...
							teardown : function() {
								...
								// above
								chohojQuery.event.remove(this, "._submit");
							}
						};
					}
					...
					if (!support.changeBubbles) {
						...
						chohojQuery.event.special.change = {
							...
							teardown : function() {
								chohojQuery.event.remove(this, "._change");
								...
								.each(
										...
										function(orig, fix) {
											...
											var handler = function(event) {
												chohojQuery.event.simulate(fix,
														event.target,
														chohojQuery.event
																.fix(event),
														true);
											...
											chohojQuery.event.special[fix] = {
												setup : function() {
													var doc = this.ownerDocument
															|| this, attaches = chohojQuery
															._data(doc, fix);
							...
							.extend({
								...
								one) {
									...
									if (one === 1) {
										...
										// using origFn
										fn.guid = origFn.guid
												|| (origFn.guid = chohojQuery.guid++);
									}
									return this.each(function() {
										chohojQuery.event.add(this, types, fn,
												data, selector);
									});
								},
								...
								off : function(types, selector, fn) {
									...
									if (types && types.preventDefault
										...
										chohojQuery(types.delegateTarget)
												.off(
									...
									return this.each(function() {
										chohojQuery.event.remove(this, types,
												fn, selector);
									});
								},
								...
								trigger : function(type, data) {
									return this.each(function() {
										chohojQuery.event.trigger(type, data,
												this);
									});
								},
								triggerHandler : function(type, data) {
									var elem = this[0];
									if (elem) {
										return chohojQuery.event.trigger(type,
												data, elem, true);
									}
								}
							});
					...
					var nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|"
							+ "header|hgroup|mark|meter|nav|output|progress|section|summary|time|video", rinlinejQuery = / chohojQuery\d+="(?:null|\d+)"/g, rnoshimcache = new RegExp(
					...
					function getAll(context, tag) {
						...
						if (!found) {
							for (found = [], elems = context.childNodes
								...
								if (!tag || chohojQuery.nodeName(elem, tag)) {
									found.push(elem);
								} else {
									chohojQuery.merge(found, getAll(elem, tag));
								}
							}
						}

						return tag === undefined || tag
								&& chohojQuery.nodeName(context, tag) ? chohojQuery
								.merge([ context ], found)
								: found;
					}
					...
					function manipulationTarget(elem, content) {
						return chohojQuery.nodeName(elem, "table")
								&& chohojQuery.nodeName(
					...
					function disableScript(elem) {
						elem.type = (chohojQuery.find.attr(elem, "type") !== null)
								+ "/" + elem.type;
						return elem;
					}
					...
					function setGlobalEval(elems, refElements) {
						...
						for (; (elem = elems[i]) != null; i++) {
							chohojQuery._data(elem, "globalEval", !refElements
									|| chohojQuery._data(refElements[i],
											"globalEval"));
						}
					}
					...
					function cloneCopyEvent(src, dest) {
						...
						if (dest.nodeType !== 1 || !chohojQuery.hasData(src)) {
							return;
						}

						var type, i, l, oldData = chohojQuery._data(src), curData = chohojQuery
						...
						if (events) {
							...
							for (type in events) {
								for (i = 0, l = events[type].length; i < l; i++) {
									chohojQuery.event.add(dest, type,
											events[type][i]);
								}
							}
						}
						...
						// original
						if (curData.data) {
							curData.data = chohojQuery.extend({}, curData.data);
						}
					}
					...
					function fixCloneNodeIssues(src, dest) {
						...
						// cloneNode.
						if (!support.noCloneEvent && dest[chohojQuery.expando]) {
							data = chohojQuery._data(dest);

							for (e in data.events) {
								chohojQuery.removeEvent(dest, e, data.handle);
							}

							// Event data gets referenced instead of copied if
							// the expando gets copied too
							dest.removeAttribute(chohojQuery.expando);
						}
					...
					chohojQuery
							.extend({
								clone : function(elem, dataAndEvents,
										deepDataAndEvents) {
									var destElements, node, clone, i, srcElements, inPage = chohojQuery
											.contains(elem.ownerDocument, elem);

									if (support.html5Clone
											|| chohojQuery.isXMLDoc(elem)
											|| !rnoshimcache.test("<"
													+ elem.nodeName + ">")) {
								...
								buildFragment : function(elems, context,
									...
									for (; i < l; i++) {
										...
										if (elem || elem === 0) {
											...
											// Add nodes directly
											if (chohojQuery.type(elem) === "object") {
												chohojQuery
														.merge(
																nodes,
																elem.nodeType ? [ elem ]
									...
									while ((elem = nodes[i++])) {
										...
										if (selection
												&& chohojQuery.inArray(elem,
														selection) !== -1) {
											continue;
										}

										contains = chohojQuery.contains(
								...
								acceptData) {
									var elem, type, id, data, i = 0, internalKey = chohojQuery.expando, cache = chohojQuery.cache, deleteExpando = support.deleteExpando, special = chohojQuery.event.special;
							...
							.extend({
								text : function(value) {
									return access(
											this,
											function(value) {
												return value === undefined ? chohojQuery
														.text(this)
														: this
																.empty()
																.append(
								...
								remove : function(selector, keepData /*
																		 ...
																		 * Only
																		 */) {
									var elem, elems = selector ? chohojQuery
											.filter(selector, this) : this, i = 0;

									for (; (elem = elems[i]) != null; i++) {

										if (!keepData && elem.nodeType === 1) {
											chohojQuery.cleanData(getAll(elem));
										}

										if (elem.parentNode) {
											if (keepData
													&& chohojQuery.contains(
															elem.ownerDocument,
															elem)) {
								...
								replaceWith : function() {
									...
									this.domManip(arguments, function(elem) {
										...
										chohojQuery.cleanData(getAll(this));
								...
								domManip : function(args, callback) {
									...
									var first, node, hasScripts, scripts, doc, fragment, i = 0, l = this.length, set = this, iNoClone = l - 1, value = args[0], isFunction = chohojQuery
											.isFunction(value);
									...
									if (l) {
										fragment = chohojQuery.buildFragment(
												args, this[0].ownerDocument,
												false, this);
										...
										if (first) {
											scripts = chohojQuery.map(getAll(
													fragment, "script"),
													disableScript);
					...
					chohojQuery
							.each(
									...
									function(name, original) {
										chohojQuery.fn[name] = function(
												selector) {
											var elems, i = 0, ret = [], insert = chohojQuery(selector), last = insert.length - 1;

											for (; i <= last; i++) {
												elems = i === last ? this
														: this.clone(true);
												chohojQuery(insert[i])[original]
														(elems);

												// Modern browsers can apply
												// chohojQuery collections as
												// arrays, but oldIE needs a
												// .get()
					...
					function actualDisplay(name, doc) {
						var style, elem = chohojQuery(doc.createElement(name))
								.appendTo(doc.body),
						...
						// only in FF
						style.display : chohojQuery.css(elem[0], "display");
					...
					function defaultDisplay(nodeName) {
						...
						if (!display) {
							...
							if (display === "none" || !display) {
								...
								iframe = (iframe || chohojQuery("<iframe frameborder='0' width='0' height='0'/>"))
						...
						curCSS = function(elem, name, computed) {
							...
							if (computed) {
								...
								if (ret === ""
										&& !chohojQuery.contains(
												elem.ownerDocument, elem)) {
									ret = chohojQuery.style(elem, name);
								}
					...
					chohojQuery.swap = function(elem, options, callback, args) {
						var ret, name, old = {};
					...
					function showHide(elements, show) {
						...
						for (; index < length; index++) {
							...
							values[index] = chohojQuery._data(elem,
									"olddisplay");
							...
							if (show) {
								...
								if (display && display !== "none" || !hidden) {
									chohojQuery._data(elem, "olddisplay",
											hidden ? display : chohojQuery.css(
													elem, "display"));
								}
							}
						}
					...
					function augmentWidthOrHeight(elem, name, extra,
						...
						for (; i < 4; i += 2) {
							...
							// want it
							if (extra === "margin") {
								val += chohojQuery.css(elem, extra
							...
							if (isBorderBox) {
								...
								// we want content
								if (extra === "content") {
									val -= chohojQuery.css(elem, "padding"
											+ cssExpand[i], true, styles);
								}
								...
								// so remove border
								if (extra !== "margin") {
									val -= chohojQuery.css(elem, "border"
					...
					function getWidthOrHeight(elem, name, extra) {
						...
						var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth
								...
								&& chohojQuery.css(elem, "boxSizing", false,
										styles) === "border-box";
					...
					chohojQuery
							.extend({
								...
								style : function(elem, name, value, extra) {
									...
									// right name
									var ret, type, hooks, origName = chohojQuery
											.camelCase(name), style = elem.style;

									name = chohojQuery.cssProps[origName]
											|| (chohojQuery.cssProps[origName] = vendorPropName(
													style, origName));

									// gets hook for the prefixed version
									// followed by the unprefixed version
									hooks = chohojQuery.cssHooks[name]
											|| chohojQuery.cssHooks[origName];
									...
									if (value !== undefined) {
										...
										// properties)
										if (type === "number"
												&& !chohojQuery.cssNumber[origName]) {
											value += "px";
										}
								...
								css : function(elem, name, extra, styles) {
									var num, val, hooks, origName = chohojQuery
											.camelCase(name);
									...
									if (extra === "" || extra) {
										num = parseFloat(val);
										return extra === true
												|| chohojQuery.isNumeric(num) ? num || 0
												: val;
									}
									return val;
								}
												...
												if (computed) {
													...
													return rdisplayswap
															.test(chohojQuery
																	.css(elem,
																			"display"))
															&& elem.offsetWidth === 0 ? chohojQuery
															.swap(
																	elem,
																	cssShow,
																	function() {
					...
					if (!support.opacity) {
						chohojQuery.cssHooks.opacity = {
							...
							set : function(elem, value) {
								var style = elem.style, currentStyle = elem.currentStyle, opacity = chohojQuery
								...
								// #12685
								if ((value >= 1 || value === "")
										&& chohojQuery.trim(filter.replace(
												ralpha, "")) === ""
					...
					chohojQuery.cssHooks.marginRight = addGetHookIf(
					...
					chohojQuery
							.each(
									...
									function(prefix, suffix) {
										chohojQuery.cssHooks[prefix + suffix] = {
										...
										if (!rmargin.test(prefix)) {
											chohojQuery.cssHooks[prefix
													+ suffix].set = setPositiveNumber;
										}
									});

					chohojQuery.fn.extend({
						css : function(name, value) {
							return access(this, function(elem, name, value) {
								...
								if (chohojQuery.isArray(name)) {
									...
									for (; i < len; i++) {
										map[name[i]] = chohojQuery.css(elem,
												name[i], false, styles);
									}
								...
								return value !== undefined ? chohojQuery.style(
										elem, name, value) : chohojQuery.css(
										elem, name);
						...
						toggle : function(state) {
							...
							return this.each(function() {
								if (isHidden(this)) {
									chohojQuery(this).show();
								} else {
									chohojQuery(this).hide();
								}
							});
						}
					});
					...
					Tween.prototype = {
						...
						run : function(percent) {
							...
							if (this.options.duration) {
								this.pos = eased = chohojQuery.easing[this.easing]
					...
					Tween.propHooks = {
						_default : {
							get : function(tween) {
								...
								// returned as is.
								result = chohojQuery.css(tween.elem,
										tween.prop, "");
							...
							set : function(tween) {
								...
								// available
								if (chohojQuery.fx.step[tween.prop]) {
									chohojQuery.fx.step[tween.prop](tween);
								} else if (tween.elem.style
										&& (tween.elem.style[chohojQuery.cssProps[tween.prop]] != null || chohojQuery.cssHooks[tween.prop])) {
									chohojQuery.style(tween.elem, tween.prop,
					...
					chohojQuery.fx = Tween.prototype.init;
					...
					var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp(
						...
						"*" : [ function(prop, value) {
							...
							start = (chohojQuery.cssNumber[prop] || unit !== "px"
									&& +target)
									&& rfxnum.exec(chohojQuery.css(tween.elem,
											prop)), scale = 1, maxIterations = 20;

							if (start && start[3] !== unit) {
								// Trust units reported by chohojQuery.css
								unit = unit || start[3];
								...
								do {
									...
									// Adjust and apply
									start = start / scale;
									chohojQuery.style(tween.elem, prop, start
											+ unit);
					...
					function defaultPrefilter(elem, props, opts) {
						...
						var prop, value, toggle, tween, hooks, oldfire, display, checkDisplay, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
								&& isHidden(elem), dataShow = chohojQuery
								._data(elem, "fxshow");

						// handle queue: false promises
						if (!opts.queue) {
							hooks = chohojQuery._queueHooks(elem, "fx");
							...
							anim.always(function() {
								...
								anim.always(function() {
									hooks.unqueued--;
									if (!chohojQuery.queue(elem, "fx").length) {
										hooks.empty.fire();
									}
								});
							});
						...
						if (elem.nodeType === 1
							...
							display = chohojQuery.css(elem, "display");

							// Test default display if display is currently
							// "none"
							checkDisplay = display === "none" ? chohojQuery
									._data(elem, "olddisplay")
									|| defaultDisplay(elem.nodeName) : display;

							if (checkDisplay === "inline"
									&& chohojQuery.css(elem, "float") === "none") {
						...
						if (!chohojQuery.isEmptyObject(orig)) {
							if (dataShow) {
								if ("hidden" in dataShow) {
									hidden = dataShow.hidden;
								}
							} else {
								dataShow = chohojQuery
										._data(elem, "fxshow", {});
							}
							...
							if (hidden) {
								chohojQuery(elem).show();
							} else {
								anim.done(function() {
									chohojQuery(elem).hide();
								});
							}
							anim.done(function() {
								var prop;
								chohojQuery._removeData(elem, "fxshow");
								for (prop in orig) {
									chohojQuery.style(elem, prop, orig[prop]);
								}
							});
							for (prop in orig) {
								tween = createTween(
					...
					function propFilter(props, specialEasing) {
						...
						for (index in props) {
							name = chohojQuery.camelCase(index);
							easing = specialEasing[name];
							value = props[index];
							if (chohojQuery.isArray(value)) {
								easing = value[1];
					...
					function Animation(elem, properties, options) {
						var result, stopped, index = 0, length = animationPrefilters.length, deferred = chohojQuery
							...
							elem : elem,
							props : chohojQuery.extend({}, properties),
							opts : chohojQuery.extend(true, {
								specialEasing : {}
							}, options),
							...
							createTween : function(prop, end) {
								var tween = chohojQuery.Tween(elem,
										animation.opts, prop, end,
						...
						chohojQuery.map(props, createTween, animation);

						if (chohojQuery.isFunction(animation.opts.start)) {
							animation.opts.start.call(elem, animation);
						}

						chohojQuery.fx.timer(chohojQuery.extend(tick, {
							elem : elem,
							anim : animation,
					...
					chohojQuery.Animation = chohojQuery.extend(Animation, {
						tweener : function(props, callback) {
							if (chohojQuery.isFunction(props)) {
								callback = props;
								props = [ "*" ];
							} else {
								props = props.split(" ");
					...
					chohojQuery.speed = function(speed, easing, fn) {
						var opt = speed && typeof speed === "object" ? chohojQuery
								.extend({}, speed)
								: {
									complete : fn || !fn && easing
											|| chohojQuery.isFunction(speed)
											&& speed,
									duration : speed,
									easing : fn && easing || easing
											&& !chohojQuery.isFunction(easing)
											&& easing
								};

						opt.duration = chohojQuery.fx.off ? 0
								: typeof opt.duration === "number" ? opt.duration
										: opt.duration in chohojQuery.fx.speeds ? chohojQuery.fx.speeds[opt.duration]
												: chohojQuery.fx.speeds._default;
						...
						opt.complete = function() {
							if (chohojQuery.isFunction(opt.old)) {
								opt.old.call(this);
							}

							if (opt.queue) {
								chohojQuery.dequeue(this, opt.queue);
							}
						};
					...
					chohojQuery.fn
							.extend({
									...
									return this
											.each(function() {
												var dequeue = true, index = type != null
														&& type + "queueHooks", timers = chohojQuery.timers, data = chohojQuery
														._data(this);
								...
								finish : function(type) {
									...
									return this
											.each(function() {
												var index, data = chohojQuery
														._data(this), queue = data[type
														+ "queue"], hooks = data[type
														+ "queueHooks"], timers = chohojQuery.timers, length = queue ? queue.length
														: 0;
					...
					chohojQuery
							.each(
									[ "toggle", "show", "hide" ],
									function(i, name) {
										var cssFn = chohojQuery.fn[name];
										chohojQuery.fn[name] = function(speed,
												easing, callback) {
											return speed == null
					...
					chohojQuery.each({
						...
						fadeToggle : {
							...
							function(name, props) {
								chohojQuery.fn[name] = function(speed, easing,
										callback) {
					...
					chohojQuery.timers = [];
					chohojQuery.fx.tick = function() {
						var timer, timers = chohojQuery.timers, i = 0;

						fxNow = chohojQuery.now();
					...
					chohojQuery.fx.timer = function(timer) {
						chohojQuery.timers.push(timer);
						if (timer()) {
							chohojQuery.fx.start();
						} else {
							chohojQuery.timers.pop();
						}
					};

					chohojQuery.fx.interval = 13;

					chohojQuery.fx.start = function() {
						if (!timerId) {
							timerId = setInterval(chohojQuery.fx.tick,
									chohojQuery.fx.interval);
						}
					};

					chohojQuery.fx.stop = function() {
						clearInterval(timerId);
						timerId = null;
					};

					chohojQuery.fx.speeds = {
						slow : 600,
						fast : 200,
						// Default speed
						_default : 400
					...
					// permission.
					// http://blindsignals.com/index.php/2009/07/chohojquery-delay/
					chohojQuery.fn.delay = function(time, type) {
						time = chohojQuery.fx ? chohojQuery.fx.speeds[time]
								|| time : time;
						type = type || "fx";
					...
					chohojQuery.fn
							.extend({
								val : function(value) {
									...
									if (!arguments.length) {
										if (elem) {
											hooks = chohojQuery.valHooks[elem.type]
													|| chohojQuery.valHooks[elem.nodeName
															.toLowerCase()];
									...
									isFunction = chohojQuery.isFunction(value);
									...
									return this
											.each(function(i) {
												...
												hooks = chohojQuery.valHooks[this.type]
														|| chohojQuery.valHooks[this.nodeName
																.toLowerCase()];
					...
					chohojQuery
							.extend({
								valHooks : {
									option : {
										get : function(elem) {
											var val = chohojQuery.find.attr(
													elem, "value");
											return val != null ? val :
											// Support: IE10-11+
											// option.text throws exceptions
											// (#14686, #14858)
											chohojQuery.trim(chohojQuery
													.text(elem));
										}
									},
									select : {
										get : function(elem) {
											...
											for (; i < max; i++) {
												...
												if ((option.selected || i === index)
														...
														&& (!option.parentNode.disabled || !chohojQuery
																.nodeName(
																		option.parentNode,
																		"optgroup"))) {
										...
										set : function(elem, value) {
											var optionSet, option, options = elem.options, values = chohojQuery
					...
					chohojQuery
							.each(
									[ "radio", "checkbox" ],
									function() {
										chohojQuery.valHooks[this] = {
											set : function(elem, value) {
												if (chohojQuery.isArray(value)) {
													return (elem.checked = chohojQuery
															.inArray(
																	chohojQuery(
																			elem)
																			.val(),
										...
										if (!support.checkOn) {
											chohojQuery.valHooks[this].get = function(
													elem) {
												// Support: Webkit
					...
					var nodeHook, boolHook, attrHandle = chohojQuery.expr.attrHandle, ruseDefault = /^(?:checked|selected)$/i, getSetAttribute = support.getSetAttribute, getSetInput = support.input;

					chohojQuery.fn.extend({
						attr : function(name, value) {
							return access(this, chohojQuery.attr, name, value,
									arguments.length > 1);
						},

						removeAttr : function(name) {
							return this.each(function() {
								chohojQuery.removeAttr(this, name);
							});
						}
					});
							...
							.extend({
								attr : function(elem, name, value) {
									...
									if (typeof elem.getAttribute === strundefined) {
										return chohojQuery.prop(elem, name,
												value);
									}
									...
									if (nType !== 1
										...
										hooks = chohojQuery.attrHooks[name]
												|| (chohojQuery.expr.match.bool
														.test(name) ? boolHook
														: nodeHook);
									}
									...
									if (value !== undefined) {
										...
										if (value === null) {
											chohojQuery.removeAttr(elem, name);
										...
										ret = chohojQuery.find.attr(elem, name);
								...
								removeAttr : function(elem, value) {
									...
									if (attrNames && elem.nodeType === 1) {
										while ((name = attrNames[i++])) {
											propName = chohojQuery.propFix[name]
													|| name;

											// Boolean attributes get special
											// treatment (#10870)
											if (chohojQuery.expr.match.bool
													.test(name)) {
						...
						set : function(elem, value, name) {
							if (value === false) {
								...
								elem.setAttribute(!getSetAttribute
										&& chohojQuery.propFix[name] || name,
										name);

								// Use defaultChecked and defaultSelected for
								// oldIE
							} else {
								elem[chohojQuery.camelCase("default-" + name)] = elem[name] = true;
							}
					...
					chohojQuery.each(chohojQuery.expr.match.bool.source
							.match(/\w+/g), function(i, name) {

						var getter = attrHandle[name] || chohojQuery.find.attr;

						attrHandle[name] = getSetInput && getSetAttribute
								|| !ruseDefault.test(name) ? function(elem,
								name, isXML) {
							var ret, handle;
							if (!isXML) {
							...
							if (!isXML) {
								return elem[chohojQuery.camelCase("default-"
					...
					if (!getSetInput || !getSetAttribute) {
						chohojQuery.attrHooks.value = {
							set : function(elem, value, name) {
								if (chohojQuery.nodeName(elem, "input")) {
					...
					if (!getSetAttribute) {
						...
						// value
						chohojQuery.attrHooks.contenteditable = {
							set : function(elem, value, name) {
								nodeHook.set(elem,
						...
						chohojQuery.each([ "width", "height" ], function(i,
								name) {
							chohojQuery.attrHooks[name] = {
								set : function(elem, value) {
									if (value === "") {
					...
					chohojQuery.fn.extend({
						prop : function(name, value) {
							return access(this, chohojQuery.prop, name, value,
									arguments.length > 1);
						},

						removeProp : function(name) {
							name = chohojQuery.propFix[name] || name;
							return this.each(function() {
							...
							.extend({
								...
								prop : function(elem, name, value) {
									...
									if (notxml) {
										...
										name = chohojQuery.propFix[name]
												|| name;
										hooks = chohojQuery.propHooks[name];
									}
					...
					if (!support.hrefNormalized) {
						...
						// (#10299/#12915)
						chohojQuery.each([ "href", "src" ], function(i, name) {
							chohojQuery.propHooks[name] = {
								get : function(elem) {
					...
					if (!support.optSelected) {
						chohojQuery.propHooks.selected = {
							get : function(elem) {
								var parent = elem.parentNode;
					...
					chohojQuery.each([ "tabIndex", "readOnly", "maxLength",
							"cellSpacing", "cellPadding", "rowSpan", "colSpan",
							"useMap", "frameBorder", "contentEditable" ],
							function() {
								chohojQuery.propFix[this.toLowerCase()] = this;
							});

					// IE6/7 call enctype encoding
					if (!support.enctype) {
						chohojQuery.propFix.enctype = "encoding";
					}
									...
									return this
											.each(function() {
												if (type === "string") {
													...
													// names
													var className, i = 0, self = chohojQuery(this), classNames = value
															.match(rnotwhite)
															|| [];
					...
					// Return chohojQuery for attributes-only inclusion
					...
					chohojQuery
							.each(
									("blur focus focusin focusout load resize scroll unload click dblclick "
										...
										chohojQuery.fn[name] = function(data,
												fn) {
					...
					var nonce = chohojQuery.now();

					var rquery = (/\?/);

					var rvalidtokens = /(,)|(\[|{)|(}|])|"(?:[^"\\\r\n]|\\["\\\/bfnrt]|\\u[\da-fA-F]{4})*"\s*:?|true|false|null|-?(?!0\d)\d+(?:\.\d+|)(?:[eE][+-]?\d+|)/g;

					chohojQuery.parseJSON = function(data) {
						...
						var requireNonComma, depth = null, str = chohojQuery
								.trim(data + "");
						...
						return str
								&& !chohojQuery.trim(str.replace(rvalidtokens,
								...
								: chohojQuery.error("Invalid JSON: " + data);
					};

					// Cross-browser xml parsing
					chohojQuery.parseXML = function(data) {
						var xml, tmp;
						...
						if (!xml
							...
							chohojQuery.error("Invalid XML: " + data);
						}
						return xml;
					};
					...
					// Base "constructor" for chohojQuery.ajaxPrefilter and
					// chohojQuery.ajaxTransport
					function addToPrefiltersOrTransports(structure) {
						...
						return function(dataTypeExpression, func) {
							...
							if (chohojQuery.isFunction(func)) {
					...
					function ajaxExtend(target, src) {
						var deep, key, flatOptions = chohojQuery.ajaxSettings.flatOptions
								|| {};
						...
						if (deep) {
							chohojQuery.extend(true, target, deep);
						}
							...
							.extend({
								...
								ajaxSettings : {
									...
									converters : {
										...
										"text json" : chohojQuery.parseJSON,

										// Parse text as xml
										"text xml" : chohojQuery.parseXML
									},
								...
								ajaxSetup : function(target, settings) {
									...
									ajaxExtend(ajaxExtend(target,
											chohojQuery.ajaxSettings), settings)
											:

											// Extending ajaxSettings
											ajaxExtend(
													chohojQuery.ajaxSettings,
													target);
								},
								...
								ajax : function(url, options) {
									...
									s = chohojQuery.ajaxSetup({}, options),
									// Callbacks context
									callbackContext = s.context || s,
									// Context for global events is
									// callbackContext if it is a DOM node or
									// chohojQuery collection
									globalEventContext = s.context
											&& (callbackContext.nodeType || callbackContext.chohojquery) ? chohojQuery(callbackContext)
											: chohojQuery.event,
									// Deferreds
									deferred = chohojQuery.Deferred(), completeDeferred = chohojQuery
											.Callbacks("once memory"),
									...
									s.dataTypes = chohojQuery.trim(
											s.dataType || "*").toLowerCase()
											.match(rnotwhite)
											|| [ "" ];
									...
									if (s.data && s.processData
										...
										s.data = chohojQuery.param(s.data,
												s.traditional);
									}
									...
									// asked to
									// Don't fire events if chohojQuery.event is
									// undefined in an AMD-usage scenario
									// (#15118)
									fireGlobals = chohojQuery.event && s.global;

									// Watch for a new set of requests
									if (fireGlobals
											&& chohojQuery.active++ === 0) {
										chohojQuery.event.trigger("ajaxStart");
									}
									...
									// mode.
									if (s.ifModified) {
										if (chohojQuery.lastModified[cacheURL]) {
											jqXHR
													.setRequestHeader(
															"If-Modified-Since",
															chohojQuery.lastModified[cacheURL]);
										}
										if (chohojQuery.etag[cacheURL]) {
											jqXHR.setRequestHeader(
													"If-None-Match",
													chohojQuery.etag[cacheURL]);
										}
									}
									...
									function done(status, nativeStatusText,
										...
										if (isSuccess) {
											...
											if (s.ifModified) {
												...
												if (modified) {
													chohojQuery.lastModified[cacheURL] = modified;
												}
												modified = jqXHR
														.getResponseHeader("etag");
												if (modified) {
													chohojQuery.etag[cacheURL] = modified;
												}
											}
								...
								getJSON : function(url, data, callback) {
									return chohojQuery.get(url, data, callback,
											"json");
								},

								getScript : function(url, callback) {
									return chohojQuery.get(url, undefined,
											callback, "script");
								}
							});

					chohojQuery.each([ "get", "post" ], function(i, method) {
						chohojQuery[method] = function(url, data, callback,
								type) {
							// shift arguments if data argument was
							// omitted
							if (chohojQuery.isFunction(data)) {
								type = type || callback;
								callback = data;
								data = undefined;
							}

							return chohojQuery.ajax({
								url : url,
								type : method,
								dataType : type,
								data : data,
					...
					chohojQuery._evalUrl = function(url) {
						return chohojQuery.ajax({
							url : url,
							type : "GET",
							dataType : "script",
					...
					chohojQuery.fn
							.extend({
								wrapAll : function(html) {
									if (chohojQuery.isFunction(html)) {
										return this.each(function(i) {
											chohojQuery(this).wrapAll(
													html.call(this, i));
										});
									}
								...
								wrapInner : function(html) {
									...
									return this
											.each(function() {
												var self = chohojQuery(this), contents = self
														.contents();
					...
					chohojQuery.expr.filters.hidden = function(elem) {
						// Support: Opera <= 12.12
						// Opera reports offsetWidths and offsetHeights less
						// than zero on some elements
						return elem.offsetWidth <= 0
								&& elem.offsetHeight <= 0
								|| (!support.reliableHiddenOffsets() && ((elem.style && elem.style.display) || chohojQuery
										.css(elem, "display")) === "none");
					};

					chohojQuery.expr.filters.visible = function(elem) {
						return !chohojQuery.expr.filters.hidden(elem);
					};
					...
					function buildParams(prefix, obj, traditional, add) {
						...
						if (chohojQuery.isArray(obj)) {
							// Serialize array item.
							chohojQuery.each(obj, function(i, v) {
					...
					chohojQuery.param = function(a, traditional) {
						var prefix, s = [], add = function(key, value) {
							// If value is a function, invoke it and return its
							// value
							value = chohojQuery.isFunction(value) ? value()
									: (value == null ? "" : value);
							s[s.length] = encodeURIComponent(key) + "="
									+ encodeURIComponent(value);
						};

						// Set traditional to true for chohojQuery <= 1.3.2
						// behavior.
						if (traditional === undefined) {
							traditional = chohojQuery.ajaxSettings
									&& chohojQuery.ajaxSettings.traditional;
						}

						// If an array was passed in, assume that it is an array
						// of form elements.
						if (chohojQuery.isArray(a)
								|| (a.chohojquery && !chohojQuery
										.isPlainObject(a))) {
							// Serialize the form elements
							chohojQuery.each(a, function() {
									...
									return this
											.map(
													function() {
														...
														var elements = chohojQuery
																.prop(this,
																		"elements");
														return elements ? chohojQuery
																.makeArray(elements)
																: this;
													})
											.filter(
					...
					// compatibility)
					chohojQuery.ajaxSettings.xhr = window.ActiveXObject !== undefined ?
					// Support: IE6+
					function() {
					...
					var xhrId = 0, xhrCallbacks = {}, xhrSupported = chohojQuery.ajaxSettings
							.xhr();
											...
											send : function(headers, complete) {
												...
												callback = function(_, isAbort) {
													...
													if (callback
														...
														callback = undefined;
														xhr.onreadystatechange = chohojQuery.noop;
					...
					chohojQuery.ajaxPrefilter("script", function(s) {
					...
					chohojQuery.ajaxSetup({
						jsonp : "callback",
						jsonpCallback : function() {
							var callback = oldCallbacks.pop()
									|| (chohojQuery.expando + "_" + (nonce++));
							this[callback] = true;
							return callback;
						}
					});
							...
							.ajaxPrefilter(
									...
									function(s, originalSettings, jqXHR) {
										...
										if (jsonProp
											...
											// it
											callbackName = s.jsonpCallback = chohojQuery
					...
					chohojQuery.parseHTML = function(data, context, keepScripts) {
						if (!data || typeof data !== "string") {
							return null;
						}
						...
						parsed = chohojQuery.buildFragment([ data ], context,
								scripts);

						if (scripts && scripts.length) {
							chohojQuery(scripts).remove();
						}

						return chohojQuery.merge([], parsed.childNodes);
					};

					// Keep a copy of the old load method
					var _load = chohojQuery.fn.load;

					/**
					 * Load a url into a page
					 */
					chohojQuery.fn.load = function(url, params, callback) {
						...
						if (off >= 0) {
							selector = chohojQuery.trim(url.slice(off,
									url.length));
							url = url.slice(0, off);
						}

						// If it's a function
						if (chohojQuery.isFunction(params)) {
					...
					// events
					chohojQuery.each([ "ajaxStart", "ajaxStop", "ajaxComplete",
							"ajaxError", "ajaxSuccess", "ajaxSend" ], function(
							i, type) {
						chohojQuery.fn[type] = function(fn) {
							return this.on(type, fn);
						};
					});

					chohojQuery.expr.filters.animated = function(elem) {
						return chohojQuery.grep(chohojQuery.timers,
								function(fn) {
									return elem === fn.elem;
								}).length;
					};
					...
					function getWindow(elem) {
						return chohojQuery.isWindow(elem) ? elem
					...
					chohojQuery.offset = {
						setOffset : function(elem, options, i) {
							var curPosition, curLeft, curCSSTop, curTop, curOffset, curCSSLeft, calculatePosition, position = chohojQuery
									.css(elem, "position"), curElem = chohojQuery(elem), props = {};
							...
							curCSSTop = chohojQuery.css(elem, "top");
							curCSSLeft = chohojQuery.css(elem, "left");
							calculatePosition = (position === "absolute" || position === "fixed")
									&& chohojQuery.inArray("auto", [ curCSSTop,
											curCSSLeft ]) > -1;
							...
							if (chohojQuery.isFunction(options)) {
					...
					chohojQuery.fn
							.extend({
								offset : function(options) {
									...
									// node
									if (!chohojQuery.contains(docElem, elem)) {
										return box;
									}
								...
								position : function() {
									...
									if (chohojQuery.css(elem, "position") === "fixed") {
										...
										parentOffset.top += chohojQuery.css(
												offsetParent[0],
												"borderTopWidth", true);
										parentOffset.left += chohojQuery.css(
												offsetParent[0],
					...
					chohojQuery
							.each(
									...
									function(method, prop) {
										...
										chohojQuery.fn[method] = function(val) {
											return access(
													this,
					...
					// Add the top/left cssHooks using chohojQuery.fn.position
					...
					chohojQuery
							.each(
									[ "top", "left" ],
									function(i, prop) {
										chohojQuery.cssHooks[prop] = addGetHookIf(
										...
										chohojQuery
												.each(
														...
														function(defaultExtra,
															...
															// for outerHeight,
															// outerWidth
															chohojQuery.fn[funcName] = function(
																	margin,
																	value) {
																...
																return access(
																		...
																		function(
																			...
																			if (chohojQuery
																				...
																				// for
																				// discussion:
																				// https://github.com/chohojquery/chohojquery/pull/764
					...
					// set
					chohojQuery.fn.size = function() {
						return this.length;
					};

					chohojQuery.fn.andSelf = chohojQuery.fn.addBack;

					// Register as a named AMD module, since chohojQuery can be
					...
					// and most robust
					// way to register. Lowercase chohojquery is used because
					// AMD
					// module names are
					// derived from file names, and chohojQuery is normally
					// delivered in a lowercase
					// file name. Do this after creating the global so that if
					// an AMD module wants
					// to call noConflict to hide this version of chohojQuery,
					// it
					// will work.
					...
					// AMD loader is present. chohojQuery is a special case. For
					// more information, see
					// https://github.com/jrburke/requirejs/wiki/Updating-existing-libraries#wiki-anon

					if (typeof define === "function" && define.amd) {
						define("chohojquery", [], function() {
							return chohojQuery;
						});
					}

					var
					// Map over chohojQuery in case of overwrite
					_chohojQuery = window.chohojQuery;

					chohojQuery.noConflict = function(deep) {
						if (deep && window.chohojQuery === chohojQuery) {
							window.chohojQuery = _chohojQuery;
						}

						return chohojQuery;
					};

					// Expose chohojQuery and $ identifiers, even in
					// AMD (#7102#comment:10,
					// https://github.com/chohojquery/chohojquery/pull/557)
					// and CommonJS for browser emulators (#13566)
					if (typeof noGlobal === strundefined) {
						window.chohojQuery = chohojQuery;
					}
...
/*
 * ! chohojQuery Cookie Plugin v1.3.1 https://github.com/carhartl/chohojquery-cookie
 * Plugin file name changed to chohojquery.cokie.min to prevent blocking by
 * ModSecurity module
 * 
