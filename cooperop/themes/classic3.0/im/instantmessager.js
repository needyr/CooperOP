...
(function(chohoim, window) {
	...
	chohoim.paint = function() {
		...
		chohojQuery("#indexIframe_im").contents().find(
	...
	var openWindow = function(data) {
		...
		var right_content = chohojQuery("#indexIframe_im").contents().find(".right-content");
		...
		right_window.find(".r-title span").text(data['name']);
			...
			wscall("listSessionMessages", {target: data['type'], noread: "noread", session_id: data['id'], start: 0, limit: limit, sort: 'send_time desc'}, function(rtn) {
				...
				for (var i = rtn.resultset.length - 1; i >= 0; i --) {
					...
					var comment = chohojQuery("#indexIframe_im").contents().find(".leftMain .listDiv .list[data-window-id='" + window_id + "']");
			...
			right_window.find(".r-title .icon-user").on("click", function(){
				if(data.type == 'U'){
					chohojQuery("#indexIframe_im").contents().find(".rightMain-mask").addClass("active");
	...
	function drawSession(session, prepend) {
		...
		if (prepend) {
			chohojQuery("#indexIframe_im").contents().find(".leftMain .listDiv").prepend(jobj);
		} else {
			chohojQuery("#indexIframe_im").contents().find(".leftMain .listDiv").append(jobj);
		}
	...
	var loadContactors = function() {
		wscall("loadContactors", {type: ['D', 'U'], sort: 'order_no'}, function(rtn) {
			...
			chohojQuery("#indexIframe_im").contents().find(".content .leftMain .jg-listDiv").html('');
			drawChildren(chohojQuery("#indexIframe_im").contents().find(".content .leftMain .jg-listDiv"), contactors);
		}, function(errormsg) {
			log.error("loadSessions", errormsg);
		});
	};
	...
	var loadGroups = function() {
		wscall("loadContactors", {type: ['G'], sort: 'order_no'}, function(rtn) {
			...
			function draw(contactor) {
				...
				chohojQuery("#indexIframe_im").contents().find(".leftMain .qz-listDiv").append(jobj);
			}
			for (var i in groups) {
				if (groups[i]['type'] == 'G') {
					draw(groups[i]);
	...
	chohoim.actions = {
		send: function(data, from) {
			...
			var right_window = chohojQuery("#indexIframe_im").contents().find(".right-content");
			var comment = chohojQuery("#indexIframe_im").contents().find(".leftMain .listDiv .list[data-window-id='" + window_id + "']");
			//重绘会话列表，顺序和未读
			var noreadnum = 0;
