...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="cooperop-timefield-link">
		<xsl:if test="descendant::cooperop-timefield">
			<link rel="stylesheet" type="text/css" href="{$contextpath}/theme/plugins/bootstrap-timepicker/css/bootstrap-timepicker.min.css?m={$module}"/>
			<script type="text/javascript" src="{$contextpath}/theme/scripts/controls/input/timefield.js"></script>
			<script type="text/javascript" src="{$contextpath}/theme/plugins/bootstrap-timepicker/js/bootstrap-timepicker.min.js"></script>
		</xsl:if>
	</xsl:template>
			...
			<xsl:choose>
				...
				<xsl:if test="@required ='true'">
					<i class="control-required">*</i>
						...
						<i class="control-icon fa fa-clock-o"></i>
