...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="cooperop-selecttree-link">
		<xsl:if test="descendant::cooperop-selecttree">
			<link rel="stylesheet" href="{$contextpath}/theme/plugins/ztree/css/zTreeStyle/zTreeStyle.css?m={$module}" type="text/css"></link>
		...
		<div class="cols{@cols}" crid="{@crid}">
			...
			<div class="control-content has-icon">
				<xsl:if test="@required ='true'">
				 ...
				 <i class="control-icon fa fa-sort-desc" style="margin-top: 5px;"></i>
				<input ctype="selecttree" icon="fa fa-sort-desc" style="cursor: text;border: 1px solid #d4cdcd;background-color: #fff;" class="form-control" type="text" readonly="readonly" value="{@text}" tt="tt">
					<xsl:if test="@required ='true'">
